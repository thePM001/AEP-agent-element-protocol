# Make Connector

**Registry id:** `connector-make`  
**UCB-only:** all traffic via `/ucb/v1/egress/make/**`

## Auth

Set `AEP_MAKE_API_TOKEN` in the environment. UCB injects it as Bearer on egress proxy.

## Default upstream

`https://eu1.make.com/api/v2`

Override in CCA plan `connectors.make` or task manifest egress routes.

## MCP / Nango

Optional MCP servers from [awesome-mcp-servers](https://github.com/punkpeye/awesome-mcp-servers) or self-hosted [Nango](https://github.com/nangohq/nango) may sit upstream - still reached only through UCB egress.
