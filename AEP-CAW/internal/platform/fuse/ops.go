...
// resolveReal maps a FUSE path under realRoot with fail-closed containment.
// Returns (path, 0) or ("", -EACCES) on escape.
func (f *fuseFS) resolveReal(path string) (string, int) {
	root := filepath.Clean(f.realRoot)
	if root == "" || root == "." {
		return "", -fuse.EACCES
	}
	...
	if path == "/" || path == "" {
		...
		// Strip absolute-leading so Join never discards root (Go Join semantics).
	...
	if !pathutil.IsRealPathUnder(candidate, root) {
		return "", -fuse.EACCES
	}
	return candidate, 0
}

// ensureContained resolves path then requires final (symlink-eval) target under root.
func (f *fuseFS) ensureContained(path string) (string, int) {
	real, errno := f.resolveReal(path)
	if errno != 0 {
		return "", errno
	}
	root := filepath.Clean(f.realRoot)
	if info, err := os.Lstat(real); err == nil && info.Mode()&os.ModeSymlink != 0 {
		...
		if !pathutil.IsRealPathUnder(target, root) {
			return "", -fuse.EACCES
		}
		return target, 0
	}
	if resolved, err := filepath.EvalSymlinks(real); err == nil {
		...
		if !pathutil.IsRealPathUnder(resolved, root) {
			return "", -fuse.EACCES
		}
...
// realPath is the contained mapping used by FUSE ops. Empty string means deny (escape).
func (f *fuseFS) realPath(path string) string {
	p, errno := f.ensureContained(path)
	if errno != 0 {
		return ""
	}
	return p
...
func (f *fuseFS) checkPolicy(fusePath string, operation platform.FileOperation) platform.Decision {
	if f.cfg.PolicyEngine == nil {
		// Fail closed: missing policy engine must never allow.
		return platform.DecisionDeny
	}
	real, errno := f.resolveReal(fusePath)
	if errno != 0 {
		return platform.DecisionDeny
	}
	return f.cfg.PolicyEngine.CheckFile(real, operation)
}
...
func (f *fuseFS) Getattr(path string, stat *fuse.Stat_t, fh uint64) int {
	...
	// Stat policy gate (was skipped entirely).
	if f.checkPolicy(path, platform.FileOpRead) == platform.DecisionDeny {
		return -fuse.EACCES
	}
...
func (f *fuseFS) Open(path string, flags int) (int, uint64) {
	...
	// Prefer not following a final symlink that escaped (ensureContained already checked).
...
func (f *fuseFS) Symlink(target string, newpath string) int {
	...
	// Reject symlink targets that escape the workspace (absolute host paths).
	if filepath.IsAbs(target) {
		root := filepath.Clean(f.realRoot)
		if !pathutil.IsRealPathUnder(filepath.Clean(target), root) {
			return -fuse.EACCES
		}
	} else if strings.HasPrefix(filepath.Clean(target), "..") {
		return -fuse.EACCES
	}
...
func (f *fuseFS) Readlink(path string) (int, string) {
	...
	realPath, errno := f.resolveReal(path)
	if errno != 0 {
		return errno, ""
	}
