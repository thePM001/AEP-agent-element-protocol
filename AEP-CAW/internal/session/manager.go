...
type Session struct {
	...
	Policy         string
	// OperatorID binds the session to the creating principal (API key hash or OIDC subject).
	// Used to prevent cross-operator session IDOR on multi-key deployments.
	OperatorID string
...
// SetOperatorID records the creating principal for ownership checks.
func (s *Session) SetOperatorID(operatorID string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.OperatorID = operatorID
}

// GetOperatorID returns the session owner principal id.
func (s *Session) GetOperatorID() string {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.OperatorID
}
