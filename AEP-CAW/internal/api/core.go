...
func (a *App) setupSeccompWrapper(req types.ExecRequest, sessionID string, s *session.Session) *wrapperSetupResult {
	...
	if _, err := exec.LookPath(wrapperBin); err != nil {
		// HIGH: fail closed when unix_sockets enforcement is enabled
		slog.Error("seccomp wrapper unavailable: refusing exec (fail closed)",
	...
	if sp == nil {
		slog.Error("seccomp wrapper disabled: notify socket pair failed (fail closed)",
			"session_id", sessionID,
			"command", origCommand)
		return nil
	}
	...
	var sessionPolicy *policy.Engine
	if s != nil {
		sessionPolicy = a.policyEngineFor(s)
	} else {
	...
	sessIDForInject := sessionID
	if s != nil {
		sessIDForInject = s.ID
	}
	envInject := a.mergeEnvInjectForSession(context.Background(), sessIDForInject, sessionPolicy)
	if envInject == nil {
...
func (a *App) createSessionWithProfile(ctx context.Context, req types.CreateSessionRequest) (types.Session, int, error) {
	...
	if req.ProjectRoot != "" {
		// Same hard guard as createSessionCore: block "/" and ".." before policy glob expand.
		if err := rejectClientPathExpand("project_root", req.ProjectRoot); err != nil {
	...
	if req.Home != "" {
		if err := rejectClientPathExpand("home", req.Home); err != nil {
			a.cleanupCreatedSession(s)
			return types.Session{}, http.StatusBadRequest, err
		}
...
func rejectClientPathExpand(label, p string) error {
	...
	if strings.Contains(p, "..") {
		return fmt.Errorf("%s must not contain parent-directory segments", label)
	}
	return nil
}
...
func (a *App) createSessionCore(ctx context.Context, req types.CreateSessionRequest) (types.Session, int, error) {
	...
	// HOME expands policy globs; reject filesystem root and parent segments.
	if req.Home != "" {
		if err := rejectClientPathExpand("home", req.Home); err != nil {
			return types.Session{}, http.StatusBadRequest, err
		}
...
func (a *App) execInSessionCore(ctx context.Context, id string, req types.ExecRequest) (*types.ExecResponse, int, error) {
	...
	if !a.canAccessSessionCtx(ctx, s) {
		return nil, http.StatusForbidden, errors.New("session access denied (operator ownership)")
	}
	...
	if pre.PolicyDecision == types.DecisionApprove && pre.EffectiveDecision == types.DecisionApprove {
		if a.approvals == nil {
			// Fail closed: approve without a manager is not allow.
		...
		if pkgErr != nil {
			...
			case pkgcheck.VerdictApprove:
				if a.approvals == nil {
					...
					pre.EffectiveDecision = types.DecisionDeny
					pre.Message = "package install approval required but approvals manager is not configured"
				} else if a.approvals != nil {
					apr := approvals.Request{
	...
	if wrapperResult == nil {
		return nil, http.StatusServiceUnavailable, fmt.Errorf("seccomp wrapper required but unavailable (fail closed)")
	}
