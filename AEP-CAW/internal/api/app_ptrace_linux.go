...
func (a *App) initPtraceTracer() {
	...
	if !ptraceInjectProbe() {
		// HIGH: fail closed when ptrace is enabled but inject probe fails.
		// Operators must set sandbox.ptrace.enabled:false if they accept no ptrace.
		slog.Error("ptrace syscall injection is unreliable on this kernel; refusing ptrace-enabled start (fail closed). " +
			"Set sandbox.ptrace.enabled:false to run without ptrace, or use a kernel with working ptrace injection.")
