...
//
// Routing logic:
//  1. No policy -> deny with "no_policy" event (fail closed).
...
func (h *FileHandler) Handle(req FileRequest) (FileResult, *types.Event) {
	...
	// 1. No policy configured - fail closed.
	if h.policy == nil {
		...
		return FileResult{Action: ActionDeny}, h.buildFileEvent(req, dec, false, false)
	}
	...
	// TM-07: soft_delete without trash divert must deny on seccomp FileHandler
	// (engine maps soft_delete -> EffectiveDecision=allow; match ptrace fail-closed).
	if (dec.Decision == "soft_delete" || dec.EffectiveDecision == "soft_delete") &&
		(req.Operation == "delete" || req.Operation == "rmdir" || req.Operation == "unlink" || req.Operation == "unlinkat") {
		if h.enforce {
			dec.Decision = "soft_delete"
			dec.EffectiveDecision = "deny"
			dec.Rule = "soft_delete_no_trash_backend"
