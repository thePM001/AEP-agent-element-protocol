...
func TestResolveViaProcRoot_Symlink(t *testing.T) {
	dir := t.TempDir()
	...
	// Resolve through current process root (pid of self).
	resolved, err := resolveViaProcRoot(os.Getpid(), link)
	if err != nil {
		t.Fatalf("resolveViaProcRoot: %v", err)
	}
	// Final path should be the target (or equivalent cleaned path).
	want, _ := filepath.EvalSymlinks(link)
	if filepath.Clean(resolved) != filepath.Clean(want) && resolved != target {
		// Accept if resolved equals real path of target
		realTarget, _ := filepath.EvalSymlinks(target)
		if filepath.Clean(resolved) != filepath.Clean(realTarget) {
			t.Fatalf("resolved %q, want target-like %q (eval %q)", resolved, target, want)
		}
	}
}
