...
func ServeNotify(ctx context.Context, fd *os.File, sessID string, pol *policy.Engine, emit Emitter) {
	...
	for {
		...
		// TM-05: fail closed - only allow after successful sockaddr parse + policy allow.
		allow := false
...
func ServeNotifyWithExecve(ctx context.Context, fd *os.File, sessID string, pol *policy.Engine, emit Emitter, execveHandler *ExecveHandler, fileHandler *FileHandler, blockList *BlockListConfig) {
	...
	for {
		...
		// TM-05: nil policy engine must fail closed for unix traps
		if pol == nil {
			slog.Debug("ServeNotifyWithExecve: nil policy, denying unix socket syscall", "session_id", sessID)
			if err := NotifRespondDeny(int(scmpFD), req.ID, int32(unix.EACCES)); err != nil {
				slog.Error("unix socket: deny response failed (nil policy)", "error", err)
			}
			continue
		}
...
func handleFileNotification(goCtx context.Context, fd seccomp.ScmpFd, req *seccomp.ScmpNotifReq, h *FileHandler, sessID string) {
	...
	if args.Nr == unix.SYS_OPENAT2 && fileArgs.HowPtr != 0 {
		...
		if err != nil {
			slog.Debug("file handler: failed to read open_how, denying", "pid", pid, "error", err)
			if err := NotifRespondDeny(int(fd), req.ID, int32(unix.EACCES)); err != nil {
				slog.Error("file handler: deny response failed", "pid", pid, "error", err)
			}
			return
		}
	...
	if err != nil {
		...
		if err != nil {
			// TM-04: fail closed when path cannot be resolved under file policy
