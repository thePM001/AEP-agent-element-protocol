...
func resolvePathAtImpl(pid int, dirfd int32, pathPtr uint64, useFallback bool) (string, error) {
	...
	// Build absolute path first (lexical join for relative paths).
	var abs string
	if filepath.IsAbs(path) {
		...
		if dirfd == atFDCWD {
			...
			abs = filepath.Clean(filepath.Join(dirPath, path))
		}
	}

	// TM-06: resolve through /proc/<pid>/root so policy sees the final path
	// after symlink walk (not merely the lexical path).
	if resolved, rerr := resolveViaProcRoot(pid, abs); rerr == nil {
		return resolved, nil
	} else if !os.IsNotExist(rerr) {
		// Fail closed on EACCES/ELOOP/etc so policy is not evaluated on a weak path.
		return "", fmt.Errorf("resolve via /proc/%d/root %q: %w", pid, abs, rerr)
	}
	// Nonexistent leaf (create/unlink target): keep absolute cleaned path.
	return abs, nil
}

// resolveViaProcRoot walks path components under /proc/<pid>/root following
// symlinks in the tracee's mount namespace (same model as ptrace resolveViaProc).
func resolveViaProcRoot(pid int, path string) (string, error) {
	...
	for len(components) > 0 {
		...
		if fi.Mode()&os.ModeSymlink != 0 {
			...
			if linkCount > maxLinks {
				return "", fmt.Errorf("resolveViaProcRoot: too many symlinks resolving %q", path)
			}
