...
func realRunProbeIteration(ctx context.Context) (IterationResult, error) {
	...
	// Closing notifyFD should wake NotifReceive (EBADF/ENOENT). On some
	// restricted hosts the ioctl does not unblock; never hang forever.
	finish := func() {
		serviceCancel()
		_ = unix.Close(notifyFD)
		select {
		...
		case <-time.After(500 * time.Millisecond):
			slog.Debug("wait_killable probe: service goroutine did not exit after notifyFD close")
		}
	}
