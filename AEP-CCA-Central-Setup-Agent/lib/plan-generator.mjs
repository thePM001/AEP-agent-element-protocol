...
import { buildUcbSetupNotes } from "./setup/ucb-env.mjs";
...
export async function generatePlanFromIntent(userIntent, dataDir, env = process.env) {
  ...
  applyAllPolicyOverrides(plan, context, env);
 ...
 * @param {object} context
 */
export function applyAllPolicyOverrides(plan, context, env = process.env) {
  ...
  applyUcbPolicyOverrides(plan, env);
}

function applyUcbPolicyOverrides(plan, env = process.env) {
  const enabledIds = new Set(plan.components.filter((c) => c.enabled).map((c) => c.id));
  if ( ! enabledIds.has("ucb") && ! plan.security  ?.  ucb_enabled) return;
  plan.policy_overrides = plan.policy_overrides   ??   {};
  plan.policy_overrides.ucb = {
    ...buildUcbSetupNotes(env),
    enabled: true,
  };
