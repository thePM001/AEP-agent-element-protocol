...
const SAFETY_RULES = {
    ...
    PROTECTED_FILES: {
        ...
        files: [
            ...
            '.claude/aep-evidence.jsonl',   // Evidence ledger is append-only (AEP 2.5)
        ],
    ...
    // -----------------------------------------------------------------------
    // RULE 8: NO TRUST SCORE MANIPULATION (AEP 2.5)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // RULE 9: NO KILL SWITCH BYPASS (AEP 2.5)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // RULE 10: NO FLEET MANIPULATION (AEP 2.5)
    // -----------------------------------------------------------------------
    NO_FLEET_MANIPULATION: {
        severity: 'CRITICAL',
        description: 'AI agent attempted to modify fleet policies or spawn without governance',
        patterns: [
            /fleet[_.]?policy\s*[:=]/gi,
            /modify[_.]?fleet/gi,
            /override[_.]?fleet/gi,
            /bypass[_.]?fleet/gi,
            /fleet[_.]?config\s*[:=]/gi,
            /spawn[_.]?ungoverned/gi,
            /disable[_.]?fleet/gi,
            /agent[_.]?limit\s*[:=]\s*(?:Infinity|9999)/gi,
            /cost[_.]?cap\s*[:=]\s*(?:Infinity|9999)/gi,
            /drift[_.]?cluster\s*[:=]\s*false/gi,
        ],
        action: 'BLOCK_AND_ALERT',
    },
};
...
class SafetyGuard {
    ...
    scanFile(filepath) {
        ...
        // Check NO_TRUST_MANIPULATION (AEP 2.5)
        ...
        // Check NO_KILL_SWITCH_BYPASS (AEP 2.5)
        ...
        // Check NO_FLEET_MANIPULATION (AEP 2.5)
        for (const pattern of SAFETY_RULES.NO_FLEET_MANIPULATION.patterns) {
            ...
            while ((match = pattern.exec(content)) !== null) {
                ...
                const line = lines[lineNum - 1] || '';
                if (line.trim().startsWith('//') || line.trim().startsWith('#') || line.trim().startsWith('*')) continue;
                this.addViolation('NO_FLEET_MANIPULATION', relPath, lineNum, match[0]);
            }
        }
    }
