...
export class TrustManager {
  ...
  applyDecay(): TrustEvent | null {
    ...
    const wholeHours = Math.floor(hoursInactive);
    const totalDecay = wholeHours * decayRate;
    if (totalDecay <= 0) return null;

    // M-38: advance activity clock so re-call does not double-count
    this.lastActivityAt += wholeHours * 1000 * 60 * 60;
    return this.applyDelta("decay", `Inactivity decay: ${wholeHours} hours`, -totalDecay);
  }
