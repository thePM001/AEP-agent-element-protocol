      ...
      if (scanResult.passed) {
        ...
        if (hardFindings.length > 0) {
          ...
          const chunk: KnowledgeChunk = {
            ...
            content: raw,
            source,
            metadata: { flagged: true, flaggedAt: new Date().toISOString() },
            scanResult,
            validated: false,
          } as KnowledgeChunk;
          chunks.push(chunk);
          ...
          // H-23: scan failed without findings - reject fail-closed (do not drop silently)
          report.rejected++;
          ledger?.append("knowledge:reject", {
            chunkId: id,
            source,
            status: "rejected",
            reason: "scan_failed_without_findings",
          });
        }
      }
    }
