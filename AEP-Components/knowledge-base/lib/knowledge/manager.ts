// AEP 2.8.6 - Knowledge Base Manager
...
import { ScannerPipeline, createDefaultPipeline } from "../../../scanners/lib/pipeline.js";
import type {
  KnowledgeBase,
  KnowledgeChunk,
...
export class KnowledgeBaseManager {
  ...
  create(name: string): KnowledgeBase {
    // M-16: refuse silent overwrite of existing base
    if (this.bases.has(name) || existsSync(this.kbDir(name))) {
  ...
  private appendChunk(kbName: string, chunk: KnowledgeChunk): void {
    ...
    // M-17: ensure meta.json exists so cold reload is not orphaned
    const metaPath = join(dir, "meta.json");
    if (!existsSync(metaPath)) {
      writeFileSync(
        metaPath,
        JSON.stringify({ name: kbName, version: "1", createdAt: new Date().toISOString() }, null, 2) + "\n"
      );
    }
    appendFileSync(
