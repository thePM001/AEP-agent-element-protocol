...
export class CodeSandbox {
  ...
  async execute(request: CodeExecutionRequest): Promise<CodeExecutionResult> {
    ...
    try {
      ...
      // Fail closed: user code is not executed. Policy-hash probe is not a sandbox.
      return {
        stdout: "",
        stderr:
          "code execution not implemented (fail closed): CodeSandbox does not run agent code",
        exitCode: 1,
        timedOut: false,
        durationMs: Date.now() - start,
      };
      ...
      return {
        stdout: "",
        stderr: `Execution failed: ${e instanceof Error ? e.message : String(e)}`, // L-1
        exitCode: 1,
