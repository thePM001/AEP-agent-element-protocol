...
use aep_lattice_channel::{
    frame_binding_from, frame_digest, DockingPort, LatticeChannelFrame,
};
use aep_lattice_crypto::verify_capsule_signature_bound;
...
fn resolve_signer_public(wire: &LatticeWire) -> Result<Vec<u8>, String> {
    let wire_hex = wire
        .signer_public_hex
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty());
    let env_list = std::env::var("AEP_WASM_TRUSTED_SIGNER_PUBLIC_HEX")
        .unwrap_or_default();
    let trusted: Vec<&str> = env_list
        .split(|c| c == ',' || c == ';' || c == ' ')
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .collect();

    // Fail closed: empty trust allowlist must not accept wire-only self-signed keys.
    if trusted.is_empty() {
        return Err(
            "wasm sandbox requires non-empty AEP_WASM_TRUSTED_SIGNER_PUBLIC_HEX".into(),
        );
    }
    let candidate = if let Some(h) = wire_hex {
        if !trusted.iter().any(|t| t.eq_ignore_ascii_case(h)) {
            return Err(
                "wasm sandbox signer_public_hex not in AEP_WASM_TRUSTED_SIGNER_PUBLIC_HEX"
                    .into(),
            );
        }
        h.to_string()
    } else if trusted.len() == 1 {
        trusted[0].to_string()
    } else {
        return Err(
            "wasm sandbox requires signer_public_hex (wire) or single AEP_WASM_TRUSTED_SIGNER_PUBLIC_HEX"
                .into(),
        );
    };
    hex::decode(candidate.trim()).map_err(|e| format!("signer_public_hex decode: {e}"))
}

fn validate_frame(wire: &LatticeWire) -> Result<(), String> {
    ...
    if frame.capsule.signature.as_ref().map(|s| s.is_empty()).unwrap_or(true) {
...
async fn handle_lattice_line(
    ...
    if let Err(e) = validate_frame(&wire) {
        return LatticeResponse {
            ok: false,
            result: None,
            status: None,
