// @PAD: p0-v275-h12-h13-eval-chain-activation-v1
// @GCDE: document_sha256=p0-v275-h12-h13-runner
// AEP 2.75 - Evaluation Chain Runner
...
export function runEvaluationChain(
  ...
  for (const step of steps) {
    ...
    if (!activation) {
      ...
      verdicts.push(
        createShortCircuitVerdict(step.step, step.name, undefined, startMs),
      );
      continue;
    }

    if (activation.mode === StepActivationMode.ALWAYS) {
      ...
      if (isHardViolation(verdict)) {
        ...
        verdict.verdict === "soft_violation" ||
        verdict.verdict === "escalate"
      ) {
        // BM-21: soft_violation / escalate must change chain decision (not silent pass)
        if (verdict.verdict === "escalate") {
          chainDecision = "deny";
          aborted = true;
        } else if (chainDecision === "allow") {
          chainDecision = "gate";
        }
        chainReasons = [verdict.verdict_reason];
        chainActionId = (verdict.details?.actionId as string) ?? chainActionId;
