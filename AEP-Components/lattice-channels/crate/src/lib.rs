...
pub fn frame_header_binding(
    ...
    format!(
        "aep-frame-v1|{channel_id}|{agent_id}|{session_id}|{port}|{contract_id}|{sent_at_unix}"
    )
    .into_bytes()
}

pub fn frame_binding_from(frame: &LatticeChannelFrame) -> Vec<u8> {
