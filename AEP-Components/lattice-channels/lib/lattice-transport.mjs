...
/** SSRF host/protocol checks (aligned with lattice-gated-fetch). */
export function assertUrlNotSsrf(urlInput, env = process.env) {
  let u;
  try {
  ...
  const allowLoop = env.AEP_LATTICE_ALLOW_LOOPBACK === "1";
  const allowPriv = env.AEP_LATTICE_ALLOW_PRIVATE === "1";
  const isLoopback =
    host === "localhost" ||
    host === "0.0.0.0" ||
    host === "::1" ||
  ...
  // IPv6 ULA fc00::/7, link-local fe80::/10, private IPv4, metadata
  const isPrivate =
    /^10\./.test(host) ||
    /^192\.168\./.test(host) ||
