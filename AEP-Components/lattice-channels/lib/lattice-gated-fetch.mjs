...
export function latticeStrictEnabled() {
  ...
  if (v === "0") {
    ...
    throw new Error("AEP_LATTICE_STRICT=0 refused outside AEP_LATTICE_STRICT_DEV=1 (fail-closed)");
  }
  return true;
}

export function assertUrlNotSsrf(urlInput) {
  let u;
  try { u = typeof urlInput === "string" ? new URL(urlInput) : urlInput; }
  catch { throw new Error("lattice-gated-fetch: invalid URL"); }
  ...
  if (!allowLoop && isLoopback) {
    throw new Error("lattice-gated-fetch: loopback blocked");
  }
  const isPrivate =
    /^10\./.test(host) ||
    /^192\.168\./.test(host) ||
  ...
  if (!allowPriv && isPrivate) {
    throw new Error("lattice-gated-fetch: private/metadata host blocked");
  }
}
...
function latticeDockRequest(socketBase, dockPort, event) {
  ...
  const signer =
    (sealed && (sealed.signer_public_hex || sealed.signerPublicHex)) ||
    "";
  if (!signer) {
