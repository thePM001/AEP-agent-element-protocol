...
import type { CovenantSpec, CovenantRule } from "../../covenant/lib/types.js";
...
export class SpawnGovernor {
  ...
  validateSpawn(
    parentId: string,
    childCovenant: CovenantSpec,
    opts?: { childRing?: number; covenantRing?: number }
  ): SpawnResult {
    ...
    // HIGH: missing parent covenant fails closed when subset is required
    if (requireSubset && !parentCovenant) {
      return {
        allowed: false,
        reason: `Parent covenant missing for "${parentId}"; spawn denied (fail-closed).`,
        childTrust: 0,
        childRing: 3,
      };
    }
    ...
    // HIGH: fixed undefined `request` crash; opts carries declared ring
    const declared = opts?.childRing ?? opts?.covenantRing;
    let childRing = Math.max(parentRing, 2) as number;
    if (typeof declared === "number" && declared >= 0 && declared <= 3) {
      childRing = Math.max(childRing, declared) as number;
    }

    // Step 4: register child in fleet (bind to parent session when known)
    const childAgentId = `child-${parentId}-${Date.now()}`;
    const parentSessionId = this.fleetManager.getSessionForAgent(parentId);
    const reg = this.fleetManager.registerAgent(
      childAgentId,
      parentId,
      parentSessionId
    );
    if (!reg.registered) {
      return {
        allowed: false,
        reason: reg.reason ?? "fleet registration failed",
        childTrust: 0,
        childRing: 3,
      };
    }
