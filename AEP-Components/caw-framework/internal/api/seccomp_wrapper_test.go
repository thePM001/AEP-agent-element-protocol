...
func TestSetupSeccompWrapper_WrapperNotFound(t *testing.T) {
	...
	// HIGH: fail closed when unix_sockets enabled and wrapper missing
	if result != nil {
		t.Fatalf("expected nil wrapper result (fail closed) when wrapper binary missing, got %+v", result)
	}
}
