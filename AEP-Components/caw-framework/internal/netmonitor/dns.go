...
func (d *DNSInterceptor) policyDecision(ctx context.Context, domain string, port int) policy.Decision {
	if d.policy == nil {
		return policy.Decision{PolicyDecision: types.DecisionDeny, EffectiveDecision: types.DecisionDeny, Rule: "no_policy", Message: "policy engine not configured"}
	}
...
func (d *DNSInterceptor) maybeApprove(ctx context.Context, commandID string, dec policy.Decision, kind string, target string) policy.Decision {
	...
	if d.approvals == nil {
		...
		dec.Message = "approval required but approvals manager is not configured"
		return dec
	}
