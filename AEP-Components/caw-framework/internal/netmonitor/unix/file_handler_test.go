...
func TestFileHandler_NilPolicy(t *testing.T) {
	...
	if result.Action != ActionDeny {
		t.Errorf("expected ActionDeny (nil policy fail-closed), got %s", result.Action)
	}
	if len(emitter.events) != 1 {
		t.Fatalf("expected 1 event, got %d", len(emitter.events))
	}
...
func TestFileHandler_NilPolicyAndEmitter(t *testing.T) {
	...
	// Should not panic; nil policy fail-closed denies
	result, _ := handler.Handle(req)
	assert.Equal(t, ActionDeny, result.Action)
}
...
func TestFileHandler_SoftDeleteWithoutTrashDenies(t *testing.T) {
	// Engine maps soft_delete -> EffectiveDecision=allow; FileHandler must still deny.
	policy := &mockFilePolicy{
		decisions: map[string]FilePolicyDecision{
			"/workspace/secret.txt": {
				Decision:          "soft_delete",
				EffectiveDecision: "allow",
				Rule:              "soft_delete_workspace",
			},
		},
	}
	emitter := &mockFileEmitter{}
	...
	result, ev := handler.Handle(req)
	if result.Action != ActionDeny {
		t.Fatalf("expected ActionDeny for soft_delete without trash, got %s", result.Action)
	}
	if result.Errno != int32(unix.EACCES) {
		t.Fatalf("expected EACCES, got %d", result.Errno)
	}
	if ev == nil {
...
func TestFileHandler_NilPolicyDenies(t *testing.T) {
	handler := NewFileHandler(nil, NewMountRegistry(), &mockFileEmitter{}, true)
	result, _ := handler.Handle(FileRequest{
		PID: 1, Syscall: int32(unix.SYS_OPENAT), Path: "/tmp/x", Operation: "open", SessionID: "s",
	})
	if result.Action != ActionDeny {
		t.Fatalf("nil policy must deny, got %s", result.Action)
	}
}
