...
func TestTransparentTCPPolicyDecisionNilPolicyDenies(t *testing.T) {
	tcp := &TransparentTCP{}
	dec := tcp.policyDecision("example.com", net.ParseIP("1.1.1.1"), 80)
	if dec.EffectiveDecision != types.DecisionDeny {
		t.Fatalf("expected deny with nil policy, got %v", dec.EffectiveDecision)
	}
}

func TestTransparentTCPMaybeApproveWithoutManagerDenies(t *testing.T) {
	tcp := &TransparentTCP{}
	...
	if out.EffectiveDecision != types.DecisionDeny {
		t.Fatalf("expected deny without approvals manager, got %v", out.EffectiveDecision)
	}
}
