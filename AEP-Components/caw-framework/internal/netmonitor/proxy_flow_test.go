...
func TestHandleHTTPAllowsAndEmitsEvents(t *testing.T) {
	...
	pol := &policy.Policy{
		...
		Name:    "allow-all",
		NetworkRules: []policy.NetworkRule{
			// CIDR-only: domain glob "*" does not match IP literals under gobwas separator '.'.
			{Name: "allow-all", CIDRs: []string{"0.0.0.0/0", "::/0"}, Decision: "allow"},
		},
	}
	engine, err := policy.NewEngine(pol, false, false)
	if err != nil {
		t.Fatalf("NewEngine: %v", err)
	}
	em := &stubEmitter{}
