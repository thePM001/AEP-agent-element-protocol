...
func TestPolicyAdapter_NilEngine(t *testing.T) {
	...
	// TM-08: nil adapter must fail closed (DecisionDeny)
	if got := adapter.CheckFile("/test", FileOpRead); got != DecisionDeny {
		t.Errorf("CheckFile on nil adapter = %v, want %v", got, DecisionDeny)
	}
	if got := adapter.CheckNetwork("example.com", 443, "tcp"); got != DecisionDeny {
		t.Errorf("CheckNetwork on nil adapter = %v, want %v", got, DecisionDeny)
	}
	if got := adapter.CheckEnv("HOME", EnvOpRead); got != DecisionDeny {
		t.Errorf("CheckEnv on nil adapter = %v, want %v", got, DecisionDeny)
	}
	if got := adapter.CheckCommand("ls", nil); got != DecisionDeny {
		t.Errorf("CheckCommand on nil adapter = %v, want %v", got, DecisionDeny)
	}
	if got := adapter.CheckRegistry(`HKLM\SOFTWARE\Test`, "query"); got != DecisionDeny {
		t.Errorf("CheckRegistry on nil adapter = %v, want %v", got, DecisionDeny)
	}
