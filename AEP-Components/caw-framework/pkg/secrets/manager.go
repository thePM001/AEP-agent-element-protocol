...
func (m *Manager) Get(ctx context.Context, req SecretRequest) (*Secret, error) {
	...
	// Check if approval is required (fail closed when approvals backend is missing).
	if m.config.RequireApproval {
		if m.approvals == nil {
			if m.auditLog != nil {
				m.auditLog.SecretAccessed(req.AgentID, req.Path, req.Provider, false)
			}
			return nil, fmt.Errorf("secrets require_approval enabled but ApprovalService is not configured (fail closed)")
		}
...
// sanitizeSecretPath rejects traversal and absolute overrides.
func sanitizeSecretPath(path string) (string, bool) {
	...
	// Reject any remaining ".." segment after clean
...
// isPathAllowed checks if a path is allowed for a provider (deny by default).
func (m *Manager) isPathAllowed(provider, path string) bool {
	clean, ok := sanitizeSecretPath(path)
	if !ok {
		return false
	}
	path = clean
	...
	// Deny by default: empty allowlists never grant access.
	return false
}
