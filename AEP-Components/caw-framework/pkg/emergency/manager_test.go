...
func withEmergencyAPIKey(t *testing.T) {
	t.Helper()
	t.Setenv("AEP_EMERGENCY_API_KEY", "test-emergency-key")
}

func authEmergencyReq(req *http.Request) {
	req.Header.Set("X-API-Key", "test-emergency-key")
}

func TestManager_HTTPHandler_Status(t *testing.T) {
	withEmergencyAPIKey(t)
	config := DefaultManagerConfig()
	m := NewManager(config, nil, nil, nil, nil)
	handler := m.HTTPHandler()

	req := httptest.NewRequest("GET", "/emergency/status", nil)
	authEmergencyReq(req)
...
func TestManager_HTTPHandler_MissingAPIKey(t *testing.T) {
	withEmergencyAPIKey(t)
	config := DefaultManagerConfig()
	m := NewManager(config, nil, nil, nil, nil)
	handler := m.HTTPHandler()
