...
type Manager struct {
	...
	// apiKey when non-empty requires X-API-Key / Bearer on HTTP routes.
	apiKey string
}
...
func NewManager(
	...
	key := strings.TrimSpace(os.Getenv("AEP_EMERGENCY_API_KEY"))
	if key == "" {
		key = strings.TrimSpace(os.Getenv("AEP_CAW_API_KEY"))
	}
	return &Manager{
...
func (m *Manager) requireHTTPAuth(w http.ResponseWriter, r *http.Request) bool {
	if m.apiKey == "" {
		http.Error(w, "emergency HTTP API key not configured (set AEP_EMERGENCY_API_KEY)", http.StatusServiceUnavailable)
		return false
	}
...
// All routes require AEP_EMERGENCY_API_KEY (or AEP_CAW_API_KEY).
