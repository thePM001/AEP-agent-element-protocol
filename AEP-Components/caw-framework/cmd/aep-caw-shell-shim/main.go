...
func main() {
	...
	// Non-interactive stdin: HIGH fail-closed default enforces policy.
	// Opt-out only with AEP_CAW_SHIM_ALLOW_NONINTERACTIVE=1 (and force not set).
	// AEP_CAW_SHIM_FORCE=1 / shim.conf force=true always enforce.
	forceShim := strings.TrimSpace(os.Getenv("AEP_CAW_SHIM_FORCE"))
	forceFromEnv := forceShim == "1"
	allowNonInteractiveBypass := strings.TrimSpace(os.Getenv("AEP_CAW_SHIM_ALLOW_NONINTERACTIVE")) == "1"
	switch {
	case forceFromEnv:
	...
	default:
		// Production default: force-enforce for non-TTY (fail-closed)
		forceShim = "1"
		debugLog("non-interactive: default fail-closed force enforce")
	}
	if !term.IsTerminal(int(os.Stdin.Fd())) && forceShim != "1" && allowNonInteractiveBypass {
		debugLog("non-interactive bypass (ALLOW_NONINTERACTIVE): executing real shell %s", realShell)
		runAndExit(realShell, argv0, os.Args[1:], os.Environ())
		return
	}
