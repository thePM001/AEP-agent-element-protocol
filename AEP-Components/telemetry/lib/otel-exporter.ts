// AEP 2.8.6 - OpenTelemetry Exporter
...
function traceIdFromSession(sessionId: string): string {
  // L-26: 128-bit hex via dual FNV-style hashes (lower collision than 32-bit djb2)
  let h1 = 0x811c9dc5 >>> 0;
  let h2 = 0x01000193 >>> 0;
  ...
  return (
    ...
    Math.imul(h1 || 1, (h2 || 1) >>> 0).toString(16).padStart(8, "0")
  ).slice(0, 32);
}
  ...
  exportEntry(entry: LedgerEntry): void {
    ...
    switch (entry.type) {
      ...
      case "action:evaluate": {
        ...
        // Short-circuit chain attributes (added by v2.75 chain runner)
