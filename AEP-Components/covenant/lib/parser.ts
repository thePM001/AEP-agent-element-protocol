...
export function parseCovenant(source: string): CovenantSpec {
  ...
  for (const line of lines) {
    ...
    if (covenantMatch) {
      // M-6: refuse silent multi-covenant merge
      if (name) {
        throw new Error(`Multiple covenants in one source not allowed (already parsing "${name}", found "${covenantMatch[1]}")`);
      }
    ...
    // L-4: only end covenant block on a sole closing brace line
    if (inBlock && line === "}") {
      inBlock = false;
      continue;
    }
