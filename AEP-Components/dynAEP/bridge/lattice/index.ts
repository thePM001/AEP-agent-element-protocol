...
export class ActionLattice {
      ...
      switch (constraint.type) {
        ...
        case "threshold": {
          if (constraint.field && constraint.condition) {
            ...
            failed.push({
              constraint,
              reason: "threshold constraint missing field or condition (fail closed)",
            });
          }
          break;
        }
        case "authorization": {
          if (constraint.field && constraint.condition) {
            ...
            failed.push({
              constraint,
              reason: "authorization constraint missing field or condition (fail closed)",
            });
          }
          break;
        }
        ...
        default:
          failed.push({ constraint, reason: `Unknown constraint type '${constraint.type}' (fail closed)` });
      }
    }
  ...
  private evaluateRequiredFieldCondition(
    ...
    // HIGH: unknown required_field conditions fail closed (was fail-open)
    return {
      passed: false,
      reason: `Unknown required_field condition '${condition}' for '${field}' (fail closed)`,
    };
  }
  ...
  private evaluateThreshold(
    ...
    return {
      passed: false,
      reason: `Unknown threshold condition '${condition}' (fail closed)`,
    };
  }
  ...
  private evaluateAuthorization(
    ...
    if (condition === "matches_registered") {
      // HIGH: require explicit registered set; non-empty string alone is not enough
      if (typeof value !== "string" || value.length === 0) {
        return { passed: false, reason: "Agent ID does not match any registered agent" };
      }
      const raw =
        payload.registered_agents ??
        payload.registered_agent_ids ??
        payload.registered;
      let list: string[] = [];
      if (Array.isArray(raw)) {
        list = raw.map(String);
      } else if (raw && typeof raw === "object") {
        list = Object.keys(raw as object);
      }
      if (list.length === 0) {
        return {
          passed: false,
          reason: "matches_registered requires server-registered agents (fail-closed)",
        };
      }
      const ok = list.includes(value);
      return {
        passed: ok,
        reason: ok ? "" : "Agent ID does not match any registered agent",
      };
    }

    // HIGH: unknown authorization conditions fail closed
    return {
      passed: false,
      reason: `Unknown authorization condition '${condition}' (fail closed)`,
    };
  }
}
...
export class LatticeFilter {
  ...
  /**
   * Mark an action as satisfied.
   * MEDIUM: default per-agent only (prevents cross-agent partial-order unlock).
   * Global satisfaction only for system_* roots or explicit agentId="global".
   */
  ...
  filter(
    ...
    // Overwrite client registered_* lists with server-side interest registry only.
    const serverPayload: Record<string, unknown> = {
      ...(event.payload || {}),
      registered_agents: Array.from(this.interests.keys()),
