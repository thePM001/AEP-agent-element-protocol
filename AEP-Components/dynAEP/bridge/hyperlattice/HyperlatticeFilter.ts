...
function defaultLatticePolicyPath(): string | null {
  ...
  try {
    ...
    const candidate = join(here, "../../policies/lattice-policy.rego");
    if (existsSync(candidate)) return candidate;
  } catch {}
  const abs = "/root/NLA-AEP-v2.8-open-source/AEP-Components/dynAEP/policies/lattice-policy.rego";
...
export class HyperlatticeFilter {
  ...
  constructor(
    ...
    // Precompiled TS tables always enforce; .rego path is reference-only until OPA is wired.
    console.info(
  ...
  private buildLatticePolicyInput(
    ...
    return {
      ...
      // HIGH: live counters (were hard-coded 0)
  ...
  async filterCrossing(
    ...
    // MEDIUM: escalate outcomes gate under strict mode
    ...
    // Production fail-closed: log_only must not force-pass (parity with SDK HyperlatticeFilter).
    // Non-production still allows log_only unless AEP_HYPERLATTICE_LOG_ONLY=0.
    const forceLogOnly =
      mode === "log_only" &&
