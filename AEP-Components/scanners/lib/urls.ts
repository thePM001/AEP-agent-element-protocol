...
export class URLScanner implements Scanner {
  ...
  /** BM-17: host equality or explicit suffix boundary (evil.example.com.attacker not match example.com). */
  private hostMatches(domain: string, rule: string): boolean {
  ...
  private evaluateURL(url: string, position: number, domain: string): Finding | null {
    ...
    if (this.blocklist.length > 0) {
      const blocked = this.blocklist.some((b) => this.hostMatches(domain, b));
      if (blocked) {
        return {
          scanner: this.name,
          severity: this.severity,
    ...
    if (this.allowlist.length > 0) {
      const allowed = this.allowlist.some((a) => this.hostMatches(domain, a));
      if (!allowed) {
        return {
          scanner: this.name,
          severity: this.severity,
