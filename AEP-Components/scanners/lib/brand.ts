// AEP 2.8.6 - Brand Compliance Scanner
  ...
  scan(content: string): Finding[] {
    ...
    for (const tm of this.trademarks) {
      ...
      while ((tmMatch = termPattern.exec(content)) !== null) {
        // M-51: allow whitespace between term and trademark suffix
        const afterMatch = content.substring(
          tmMatch.index + tmMatch[0].length,
          tmMatch.index + tmMatch[0].length + tm.suffix.length + 16
        );
        const normalized = afterMatch.replace(/^\s+/, "");
        if (!normalized.startsWith(tm.suffix) && !afterMatch.includes(tm.suffix)) {
          findings.push({
            scanner: this.name,
            severity: this.severity,
            match: tmMatch[0],
