...
export class ScannerPipeline {
   ...
   * Scanner exceptions fail the pipeline (not fail-open empty findings).
   */
  scan(content: string): ScanResult {
    ...
    for (const scanner of this.scanners) {
      try {
        const part = scanner.scan(content);
        if (!Array.isArray(part)) {
          findings.push({
            scanner: scanner.name ?? "pipeline",
            category: "scanner:exception",
            severity: "hard",
            match: "scanner returned non-array findings (fail-closed)",
            position: 0,
          });
        } else {
          findings.push(...part);
    ...
    // Pipeline fails if any finding exists (exceptions always produce hard findings)
    const hard = findings.some((f) => f.severity === "hard" || f.category === "scanner:exception");
    return {
      passed: findings.length === 0 && !hard,
      findings,
    };
  }
}
...
export function createDefaultPipeline(config?: Partial<ScannersConfig>): ScannerPipeline {
  ...
  // BM-15: prompt-injection and supply-chain wired into default pipeline
  if ((config as { prompt_injection?: { enabled?: boolean } })?.prompt_injection?.enabled !== false) {
    scanners.push(promptInjectionScanner);
  }
  if ((config as { supply_chain?: { enabled?: boolean } })?.supply_chain?.enabled !== false) {
    scanners.push(supplyChainScanner);
  }
