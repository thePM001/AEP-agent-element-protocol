// AEP 2.8.6 - Temporal Governance Scanner
    ...
    if (this.checkStaleReferences) {
      for (const dd of detectedDates) {
        if (dd.date < now) {
          ...
          // M-24: only flag past dates when present-tense claim nearby (not pure historical narrative)
          const presentClaim = /\b(is|are|currently|today|now|this year|will be)\b/i.test(context);
          if (!hasAsOf && presentClaim) {
            findings.push({
              scanner: this.name,
