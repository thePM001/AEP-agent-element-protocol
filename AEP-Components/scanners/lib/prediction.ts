// AEP 2.8.6 - Prediction Validator Scanner
  ...
  scan(content: string): Finding[] {
    ...
    if (this.requireConfidence) {
      ...
      while ((numMatch = NUMERIC_PREDICTION_PATTERN.exec(content)) !== null) {
        // H-38: qualifier must be near the match, not document-wide
        const start = Math.max(0, numMatch.index - 80);
        const end = Math.min(content.length, numMatch.index + numMatch[0].length + 80);
        const window = content.slice(start, end).toLowerCase();
        const hasQualifier = CONFIDENCE_QUALIFIERS.some(
          (q) => window.includes(q.toLowerCase())
        );
        if (!hasQualifier) {
          findings.push({
            scanner: this.name,
