...
import type { LedgerEntry } from "../../evidence-ledger/lib/ledger/types.js";
...
export class DatasetManager {
  ...
  addFromLedger(
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // M-36: skip malformed ledger lines
        continue;
      }
  ...
  importFile(path: string): Dataset {
    ...
    let data: Dataset;
    try {
      data = JSON.parse(content) as Dataset;
    } catch {
    ...
    // M-37: schema-validate each entry
    for (const [i, e] of data.entries.entries()) {
      if (!e || typeof e !== "object") throw new Error(`entries[${i}] must be object`);
      if (typeof (e as { id?: unknown }).id !== "string") throw new Error(`entries[${i}].id required`);
      if (typeof (e as { input?: unknown }).input !== "string") throw new Error(`entries[${i}].input required`);
      const outcome = (e as { expectedOutcome?: unknown }).expectedOutcome;
      if (outcome !== "pass" && outcome !== "fail") {
        throw new Error(`entries[${i}].expectedOutcome must be pass|fail`);
      }
    }
