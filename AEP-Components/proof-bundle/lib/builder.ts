...
export class ProofBundleBuilder {
  ...
  /** BM-10: deep-canonical JSON (sorted keys recursive) for sign/verify. */
  static deepCanonicalize(value: unknown): unknown {
    if (Array.isArray(value)) {
      return value.map((v) => ProofBundleBuilder.deepCanonicalize(v));
    }
    if (value !== null && typeof value === "object") {
      const obj = value as Record<string, unknown>;
      const ordered: Record<string, unknown> = {};
      for (const key of Object.keys(obj).sort()) {
        ordered[key] = ProofBundleBuilder.deepCanonicalize(obj[key]);
      }
      return ordered;
    }
    return value;
  ...
  static serializeForSigning(bundle: ProofBundle): string {
    ...
    return JSON.stringify(ProofBundleBuilder.deepCanonicalize(fields));
  }
  ...
  static signPayload(payload: string, privateKey: string): string {
    ...
    try {
      ...
      try {
        ...
        // SEC-10 / C-4: no unauthenticated SHA-256(payload+key) soft signature
        throw new Error(
          `Proof bundle signing failed (no soft hash fallback): ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  }
