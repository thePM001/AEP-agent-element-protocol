...
function safeCovenantName(name: string): string | null {
...
function resolveUnderDir(baseDir: string, fileName: string): string | null {
...
export class AEPassistant {
  ...
  private executeSetup(projectType: ProjectType, preset: GovernancePreset, enableTrust: boolean): AEPassistResponse {
    ...
    if (!enableTrust) {
      // M-3: flexible YAML trust block (indent-tolerant)
      const trustBlock = /trust:\s*\n(?:[ \t]+[^\n]*\n)*/;
      if (trustBlock.test(policyContent)) {
...
Streaming validation: ${presetConfig.streaming.enabled ? "on" : "off"}`,
  ...
  handleStatus(): AEPassistResponse {
    // H-3: resolve active policy path once (prefer .aep, else legacy)
  ...
  view    View a specific covenant`,
    ...
    switch (action) {
      ...
      case "create": {
        ...
        const name = safeCovenantName(extraArgs[0]);
        if (!name) {
          return {
            mode: "covenant",
            message: "Invalid covenant name. Use letters, digits, dot, underscore or hyphen only.",
          };
        }
...
Example:
  ...
  require trust_tier >= "standard";`,
        ...
        const filePath = resolveUnderDir(covenantsDir, `${name}.covenant`);
        if (!filePath) {
          return { mode: "covenant", message: "Covenant path rejected (path traversal)." };
        }
      ...
      case "view": {
        ...
        if (!name) {
          return {
            mode: "covenant",
            message: "Specify a valid covenant name.",
...
Formats: json, csv, html`,
    ...
    switch (lower) {
      ...
      case "html": {
        const rows = reportData.sessions
          .map(s => `<tr><td>${escapeHtml(String(s.id))}</td><td>${escapeHtml(String(s.state))}</td><td>${s.allowed}</td><td>${s.denied}</td><td>${s.gated}</td></tr>`)
          .join("\n");
