...
export class AEPAssistant {
  ...
  handleSetup(agent: AssistAgent, preset: AssistPreset, multiAgent: boolean): AssistResponse {
    ...
    switch (agent) {
      ...
      case "codex": {
        const codexDir = join(this.workDir, ".codex");
        if (!existsSync(codexDir)) mkdirSync(codexDir, { recursive: true });

        writeFileSync(
          join(this.workDir, "AGENTS.md"),
          `# AEP Governance\n\nThis project uses the Agent Element Protocol (AEP) 2.8 for governed agent interactions.\nSee docs/CODEX-INSTALL.md in the AEP repository for full setup steps.\n\n${generateCodexAgentSection()}`
        );
        ...
        filesCreated.push(join(codexDir, "config.toml"));
        break;
      }
    }
  ...
  private flipSectionEnabled(yaml: string, section: string, enabled: boolean): string {
    // M-4: only flip enabled inside the named top-level section
    const re = new RegExp(
      "(" + section + ":\\s*\\n(?:[ \\t]+[^\\n]*\\n)*?)([ \\t]+)enabled:\\s*(true|false)",
      "i"
    );
    if (re.test(yaml)) {
      return yaml.replace(re, "$1$2enabled: " + enabled);
    }
    return yaml;
  }
    ...
    try {
      ...
      if (/enable streaming/.test(lower)) {
        if (!modified.includes("streaming:")) {
          ...
          modified = this.flipSectionEnabled(modified, "streaming", true);
        }
        ...
        modified = modified.replace(/(intent:[\s\S]*?)tracking:\s*false/, "$1tracking: true");
      } else if (/disable drift|turn off drift/.test(lower)) {
        modified = modified.replace(/(intent:[\s\S]*?)tracking:\s*true/, "$1tracking: false");
        ...
        if (!modified.includes("quantum:")) {
          ...
          modified = this.flipSectionEnabled(modified, "quantum", true);
        }
