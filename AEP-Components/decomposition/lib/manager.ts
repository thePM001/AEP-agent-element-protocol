...
export class TaskDecompositionManager {
  ...
  assignAction(taskId: string, actionId: string, action?: { tool?: string; input?: Record<string, unknown> }): void {
    const task = this.taskIndex.get(taskId);
    if (!task) {
      throw new Error(`Task "${taskId}" not found.`);
    }
    // M-39: validate scope/budget before assignment
    if (action) {
      const err = this.validateActionScope(taskId, action.tool ?? "", action.input ?? {});
      if (err) throw new Error(err);
    } else {
      // still enforce action budget when tool/input omitted
      const scope = task.scope;
      if (scope && task.actionIds.length >= scope.maxActions) {
        throw new Error(`Task action budget exceeded: ${task.actionIds.length}/${scope.maxActions}.`);
      }
    }
