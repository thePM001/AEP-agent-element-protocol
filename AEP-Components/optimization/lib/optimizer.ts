// AEP 2.8.6 - Prompt Optimization Under Governance
...
import { EvalRunner } from "../../eval/lib/runner.js";
import { AgentGateway } from "../gateway.js";
import type { EvalDataset, EvalReport } from "../../eval/lib/types.js";
...
export class PromptOptimizer {
  ...
  comparePrompts(
    ...
    // M-40: bind distinct prompts into dataset metadata for each arm
    const datasetA = { ...dataset, systemPrompt: promptA, promptVariant: "A" } as typeof dataset & { systemPrompt: string };
    const datasetB = { ...dataset, systemPrompt: promptB, promptVariant: "B" } as typeof dataset & { systemPrompt: string };
    const reportA = runnerA.run(datasetA as typeof dataset, policyPath);

    const gatewayB = new AgentGateway({ ledgerDir: ".aep/eval-compare-b" });
    const runnerB = new EvalRunner(gatewayB);
    const reportB = runnerB.run(datasetB as typeof dataset, policyPath);
    (reportA as { promptUsed?: string }).promptUsed = promptA;
    (reportB as { promptUsed?: string }).promptUsed = promptB;
