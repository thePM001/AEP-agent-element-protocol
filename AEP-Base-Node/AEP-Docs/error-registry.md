...
- **Task manifests:** `AEP-Base-Node/AEP-Crate/src/task_manifest.rs` + `$AEP_DATA/ucb/manifests/`
- **Side-channel monitor:** `AEP-Base-Node/AEP-Crate/src/side_channel_monitor.rs`

## Registration

New components register in `AEP-Base-Node/AEP-Registry/catalog.json` and `AEP-Base-Node/AEP-Registry/components/<id>.json`, not in a separate top-level folder.