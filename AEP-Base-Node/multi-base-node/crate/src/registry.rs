...
mod tests {
    ...
    #[test]
    fn validate_registry_rejects_multiple_primaries() {
        // TASK-A28-H08: data-plane split-brain detection
        let mut registry = NodesRegistryV2::empty();
        ...
        registry.nodes.push(mk("p1"));
        registry.nodes.push(mk("p2"));
        let err = validate_registry(&registry).unwrap_err();
        assert!(
            matches!(err, RegistryError::Invalid(ref m) if m.contains("multiple primary")),
            "{err:?}"
        );
    }

    #[test]
    fn validate_registry_rejects_duplicate_node_id() {
        let mut registry = NodesRegistryV2::empty();
        let node = NodeRecord {
            node_id: "dup".into(),
        ...
        let err = validate_registry(&registry).unwrap_err();
        assert!(
            matches!(err, RegistryError::Invalid(ref m) if m.contains("duplicate node_id")),
            "{err:?}"
        );
    }
}