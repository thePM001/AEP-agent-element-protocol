...
impl CommerceAction {
    pub fn parse(s: &str) -> Option<Self> {
        match s {
            ...
            "checkout_start" => Some(Self::CheckoutStart),
            // HIGH: complete is distinct so spend is not double-counted on both actions
            "checkout_complete" => Some(Self::CheckoutComplete),
            "payment_negotiate" => Some(Self::PaymentNegotiate),
            "payment_authorize" => Some(Self::PaymentAuthorize),
...
impl CommerceValidationResult {
    ...
    /// Human-approval gate: never report valid until operator approval is proven.
    pub fn gate_required_fail(errors: Vec<String>) -> Self {
        Self {
            valid: false,
            errors,
            gate_required: true,
...
pub fn metadata_category(metadata: &Value) -> Option<String> {
    metadata
        .get("category")
        .or_else(|| metadata.get("product_category"))
        .or_else(|| metadata.get("productCategory"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
}

pub fn injection_scan(text: &str) -> Vec<String> {
    // LOW: broader denylist for commerce metadata/reason fields
    let patterns = [
        "<script",
        "</script",
        "javascript:",
