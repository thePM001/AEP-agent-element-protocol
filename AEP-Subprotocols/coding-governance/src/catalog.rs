...
pub fn normalize_path(path: &str) -> String {
    ...
    // HIGH: reject parent-directory traversal; collapse safe segments only
    ...
    for seg in p.split('/') {
        ...
        if seg == ".." {
            // Drop previous segment if present; never escape above root
            let _ = stack.pop();
            continue;
        }
        stack.push(seg);
...
/// True when raw path attempts `..` traversal that would escape a prefix check.
