...
use crate::token::{default_ttl_secs, issue_token_strict, mint_intent_id};
...
pub fn handle_propose(payload: &Value) -> ValidationResult {
    ...
    let token = match issue_token_strict(&intent_id, &intent, &report, default_ttl_secs()) {
        Ok(t) => t,
        Err(e) => {
            return ValidationResult::fail(vec![format!("propose token issue failed: {e}")]);
        }
    };
    if token.allowed_paths.is_empty() {
        return ValidationResult::fail(vec![
            "propose envelope allowed_paths must be non-empty".into(),
        ]);
    }
