...
fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Commerce {
            ...
            // Surface gate_required so operators never see a bare valid flag alone.
            let detail = if r.gate_required {
                Some(serde_json::json!({ "gate_required": true }))
            } else {
                None
            };
            ValidationResult {
                valid: r.valid && !r.gate_required,
                errors: r.errors,
                detail,
            }
        }
