...
export class MLEEstimator {
    ...
    for (const field of mle.fields) {
      ...
      if (field.fieldType === "numeric") {
        ...
        if (schemaMax !== undefined && field.mleMax !== undefined) {
          const denomMax = (field.mleMax === 0 || field.mleMax === undefined) ? 1 : field.mleMax;
          const ratio = schemaMax / denomMax; // H-41
          if (ratio > 2) {
        ...
        if (schemaMin !== undefined && field.mleMin !== undefined) {
          ...
          if (ratio > 2 || (schemaMin < 0 && field.mleMin >= 0)) {
            // H-39: just below observed min for any sign
            const proposed = field.mleMin - Math.abs(field.mleMin || 1) * margin;
            proposals.push({
              fieldName: field.fieldName,
  ...
  updateEstimation(
    ...
    for (const [key, value] of Object.entries(newRecord)) {
      ...
      if (field.fieldType === "numeric" && typeof value === "number") {
        // H-40: n is prior sampleCount + 1 (no stale denominator)
        const n = field.sampleCount + 1;
        const oldMean = field.mleMean ?? 0;
        const newMean = oldMean + (value - oldMean) / n;
        const oldVariance = field.mleVariance ?? 0;
        const newM2 = oldVariance * field.sampleCount + (value - oldMean) * (value - newMean);
        field.sampleCount = n;
        field.mleMean = newMean;
        field.mleVariance = n > 1 ? newM2 / (n - 1) : 0;
      ...
      } else if (field.fieldType === "enum" && typeof value === "string") {
        field.sampleCount++;
      ...
      } else if (field.fieldType === "string" && typeof value === "string") {
        field.sampleCount++;
  ...
  private finalizeEstimate(acc: FieldAccumulator): MLEFieldEstimate {
    ...
    if (fieldType === "numeric") {
      ...
      if (acc.count >= 2) {
        ...
        // M-27: full z-table for common confidence levels
        const cl = this.config.confidenceLevel ?? 0.95;
        const z =
          cl >= 0.999 ? 3.291 :
          cl >= 0.99 ? 2.576 :
          cl >= 0.98 ? 2.326 :
  ...
  private derivePattern(strings: string[]): string {
    ...
    // M-26: avoid invalid empty character class patterns
    const nonEmpty = strings.filter((s) => s.length > 0);
    if (nonEmpty.length === 0) return "^$";

    const lens = nonEmpty.map(s => s.length);
