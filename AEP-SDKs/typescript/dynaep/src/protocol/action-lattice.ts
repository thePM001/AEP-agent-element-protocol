...
export class ActionLattice {
  ...
  private evaluateAuthorization(
    ...
    if (condition === "matches_registered") {
      // HIGH: require registered_agents list; non-empty string alone is not membership
      const raw =
      ...
      const ok = id.length > 0 && list.includes(id);
      return {
        passed: ok,
...
export class LatticeFilter {
  ...
  filter(
    ...
    // 2. Check trust floor
    // TM-19 defense-in-depth: without a bound agent_id, ignore client trust_tier.
