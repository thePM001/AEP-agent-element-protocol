...
const mleValidationHook: ValidationHook = {
  ...
  async validate(
    ...
    // MEDIUM: empty attractors fail closed under production/strict
    if (attractors.length === 0) {
      const allowEmpty =
        process.env.AEP_MLE_ALLOW_EMPTY_ATTRACTORS === "1" &&
      ...
      if (!allowEmpty) {
        return {
          ...
          confidence: 1.0,
          details:
            "MLE hook: no attractors defined for this node (fail-closed)",
        };
      }
      return {
        passed: true,
        ...
        confidence: 0.2,
        details:
          "MLE hook: no attractors; allowed only with AEP_MLE_ALLOW_EMPTY_ATTRACTORS=1",
      };
    }
