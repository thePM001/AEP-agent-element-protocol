...
const noopValidationHook: ValidationHook = {
  ...
  async validate(
    ...
    // MEDIUM: refuse noop as production validator unless explicitly allowed
    if (process.env.AEP_ALLOW_NOOP_HOOK !== "1") {
      return {
        passed: false,
        score: 0,
        confidence: 1.0,
        details:
          "Noop hook denied in production (set AEP_ALLOW_NOOP_HOOK=1 for benchmarks only)",
      };
    }
    return {
      passed: true,
      score: 1.0,
      confidence: 1.0,
      details: "Noop hook: pass-through (explicit AEP_ALLOW_NOOP_HOOK=1)",
    };
  },
};
