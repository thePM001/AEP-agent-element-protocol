// @PAD: /root/dynAEP/sdk/typescript/src/persistence/BufferedLedger.ts
// ===========================================================================
...
async function sha256(input: string): Promise<string> {
  ...
  // MEDIUM: never use weak placeholder when hash chain is required
  throw new Error(
    "BufferedLedger: SHA-256 unavailable (Node crypto or Web Crypto required for hash chain)",
  );
}

/**
 * Synchronous SHA-256 using Node.js crypto (TASK-A28-H12: real hashes for verifyChain).
 */
...
export class BufferedLedger {
   ...
   */
  /**
   * Verify the hash chain integrity of flushed entries.
   * TASK-A28-H12: also detects tampered hashes (returns first broken index).
   * Unflushed buffer is not part of the durable chain; call flush() before verify
   * when crash-durability is required.
   */
  verifyChain(): number {
    if (!this.config.hashChainEnabled) {
      return -1;
    ...
    for (let i = 0; i < this.flushed.length; i++) {
      ...
      if (entry.hash) {
        ...
        // Fail closed when we cannot recompute or hash mismatches (tamper)
        if (!computed || computed !== entry.hash) {
          return i;
        }
        expectedPrev = entry.hash;
      } else {
        // Missing hash on a chain-enabled ledger is a break
        return i;
      }
    }
  ...
  /**
   * Number of unflushed entries that would be lost on process crash.
   */
