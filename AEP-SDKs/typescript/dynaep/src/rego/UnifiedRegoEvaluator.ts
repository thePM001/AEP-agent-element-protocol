// @PAD: /root/dynAEP/sdk/typescript/src/rego/UnifiedRegoEvaluator.ts
// ===========================================================================
...
export class UnifiedRegoEvaluator {
  ...
  private evaluateCli(input: RegoInput): RegoResult {
    ...
    try {
      // argv only: never shell-interpolate policy paths or JSON.
      ...
      const runOpa = (policyPath: string, query: string): Record<string, unknown> => {
        const out = execFileSync(
          "opa",
          ["eval", "-f", "json", "-d", policyPath, "-I", query],
          { encoding: "utf-8", timeout: 5000, input: inputJson }
        ) as unknown as string;
        return JSON.parse(out) as Record<string, unknown>;
      };

      const structuralParsed = runOpa(paths.structural, "data.aep.forbidden.deny");
      const temporalParsed = runOpa(paths.temporal, "data.dynaep.temporal.deny_temporal");
      const perceptionParsed = runOpa(paths.perception, "data.dynaep.perception.deny_perception");
  ...
  private async recompileWasm(_policyPaths: string[]): Promise<void> {
    ...
    try {
      const { execFileSync } = require("child_process") as {
        execFileSync: (file: string, args: string[], opts: Record<string, unknown>) => Buffer;
      };
