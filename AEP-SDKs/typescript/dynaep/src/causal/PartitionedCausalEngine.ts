// @PAD: /root/dynAEP/sdk/typescript/src/causal/PartitionedCausalEngine.ts
// ===========================================================================
...
export class PartitionedCausalEngine {
  ...
  handleCrossPartitionMove(
    ...
    try {
      ...
      try {
        // TASK-A28-H13: transfer buffered events before remove so they are not dropped.
        const pending = oldPartition.takeBufferedForElement(elementId);
        oldPartition.removeElement(elementId);
        for (const event of pending) {
          newPartition.restoreBufferedEvent(event);
        }
        ...
        for (const descId of descendants) {
          // Transfer buffered descendant events as well
          const descPending = oldPartition.takeBufferedForElement(descId);
          oldPartition.removeElement(descId);
          for (const event of descPending) {
            newPartition.restoreBufferedEvent(event);
          }
   ...
   */
  /**
   * Restore causal state from durable store on startup (TASK-A28-H11).
   * Restores vector clocks, expected agent sequences, and reorder buffer.
   */
  async restoreFromStore(): Promise<void> {
    if (!this.store) return;

    // Load vector clocks per partition and seed expected sequences
    const clocks = await this.store.loadVectorClocks();
    for (const [partitionKey, agents] of clocks) {
      const partition = this.getOrCreatePartition(partitionKey);
      partition.restoreVectorClock(agents);
    }

    // Load causal position
    this.globalDeliveryPosition = await this.store.loadCausalPosition();

    // Load agent registry (seed sequences when clock missing for an agent)
    const agents = await this.store.loadAgentRegistry();
    for (const [agentId, registration] of agents) {
      // Without partition affinity, seed every partition that already exists
      // and ensure at least a default partition for future events.
      if (this.partitions.size === 0) {
        this.getOrCreatePartition("default").restoreAgentSequence(
          agentId,
          registration.lastSequence,
        );
      } else {
        for (const [, partition] of this.partitions) {
          partition.restoreAgentSequence(agentId, registration.lastSequence);
        }
      }
    }

    // Load reorder buffer and re-buffer pending events
    const buffer = await this.store.loadReorderBuffer();
    for (const buffered of buffer) {
      const partition = this.getOrCreatePartition(buffered.partitionKey);
      partition.restoreBufferedEvent(buffered.event);
    }
  }
  ...
  getStateSnapshot(): CausalStateSnapshot {
    ...
    const reorderBuffer: BufferedEvent[] = [];
    ...
    for (const [key, partition] of this.partitions) {
      ...
      for (const event of partition.snapshotReorderBuffer()) {
        reorderBuffer.push({
          event,
          bufferedAt: Date.now(),
          partitionKey: key,
      ...
      // Export agent expected sequences into agent registry for restore
      const clockJson = vectorClocks[key];
      for (const [agentId, lastSeq] of Object.entries(clockJson)) {
        const existing = agentRegistry[agentId];
        if (!existing || lastSeq > existing.lastSequence) {
  ...
  async shutdown(): Promise<void> {
    if (this.store) {
      ...
      await this.store.saveReorderBuffer(snapshot.reorderBuffer);
      const agentMap = new Map(Object.entries(snapshot.agentRegistry));
      await this.store.saveAgentRegistry(agentMap);
