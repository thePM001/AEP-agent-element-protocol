// @PAD: /root/dynAEP/sdk/typescript/src/causal/SubtreeOrderingContext.ts
// ===========================================================================
...
export class SubtreeOrderingContext {
   ...
   * TASK-A28-H13: use takeBufferedForElement before remove so events are transferred,
   * not silently dropped.
   */
  removeElement(elementId: string): void {
    this.elementHistoryMap.delete(elementId);
    // Drop only if caller already transferred; default remove still filters buffer
  ...
  /**
   * Extract buffered events for an element (and clear timers) without dropping identity.
   * Used by cross-partition moves so pending events re-buffer on the destination partition.
   */
  takeBufferedForElement(elementId: string): CausalEvent[] {
    const taken: CausalEvent[] = [];
    const remaining: CausalEvent[] = [];
    for (const event of this.reorderBuffer) {
      if (event.targetElementId === elementId) {
        taken.push(event);
    ...
    this.reorderBuffer = remaining;
    return taken;
  }

  /**
   * Restore vector clock from persisted map (TASK-A28-H11).
   * Also seeds expectedSequence so agents resume at last+1.
   */
  restoreVectorClock(clock: Record<string, number>): void {
    this.vectorClock = new SparseVectorClock(clock);
    for (const [agentId, seq] of Object.entries(clock)) {
      if (typeof seq === "number" && seq >= 0) {
        // expected next sequence is last seen + 1
        this.expectedSequence.set(agentId, seq + 1);
      }
    }
  }

  /**
   * Re-buffer a pending event after restore or partition move (no auto-deliver).
   */
  restoreBufferedEvent(event: CausalEvent): void {
    if (this.deliveredEventIds.has(event.eventId)) {
      return;
    }
    if (this.reorderBuffer.some((e) => e.eventId === event.eventId)) {
      return;
    }
    this.bufferEvent(event);
  }
  ...
  /**
   * Register agent expected sequence without delivering events.
   */
  restoreAgentSequence(agentId: string, lastSequence: number): void {
    this.expectedSequence.set(agentId, Math.max(0, lastSequence) + 1);
  }

  /** Snapshot pending reorder buffer events for this partition. */
  snapshotReorderBuffer(): CausalEvent[] {
