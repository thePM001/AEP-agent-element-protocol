// @PAD: /root/dynAEP/sdk/typescript/src/temporal/perception-engine.ts
// ===========================================================================
...
export class PerceptionEngine {
  ...
  govern(event: TemporalOutputEvent): GovernedEnvelope {
    ...
    // TASK-A28-H10: unregistered modality must not silently pass as applied=original with no violations.
    const unknownModality = staticResult.violations.some(
      (v) => v.parameter === "modality" && v.severity === "hard",
    );
    if (unknownModality) {
      return {
        originalAnnotations,
        governedAnnotations: { ...originalAnnotations },
        adaptiveAnnotations: { ...originalAnnotations },
        applied: "governed",
