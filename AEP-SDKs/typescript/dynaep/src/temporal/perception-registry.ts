// @PAD: /root/dynAEP/sdk/typescript/src/temporal/perception-registry.ts
// ===========================================================================
...
export class PerceptionRegistry {
  ...
  validate(modality: string, annotation: TemporalAnnotation): PerceptionValidationResult {
    const profile = this.modalities.get(modality);
    if (!profile) {
      // TASK-A28-H10: unknown modality is a hard fail, not a silent empty-violation bypass.
      const unknownBound: PerceptionBounds = {
        min: 0,
        max: 0,
        comfortable_min: 0,
        comfortable_max: 0,
        unit: "count",
        source: "perception-registry",
      };
      return {
        valid: false,
        violations: [
          {
            ...
            bound: unknownBound,
            severity: "hard",
            message: "Unknown modality: " + modality + " - governance refused (fail closed)",
          },
        ],
        clamped: { ...annotation },
