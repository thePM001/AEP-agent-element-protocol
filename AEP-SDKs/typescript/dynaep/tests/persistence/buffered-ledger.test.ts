// @PAD: /root/dynAEP/sdk/typescript/tests/persistence/buffered-ledger.test.ts
// ===========================================================================
...
test("verifyChain detects tampered hash", () => {
  const clock = createClock();
  const ledger = new BufferedLedger(clock, { hashChainEnabled: true });
  ledger.record("accepted", "CP-00001", "test1");
  ledger.record("accepted", "CP-00002", "test2");
  ledger.flush();
    // Mutate via serialize/deserialize with corrupted hash
  const data = JSON.parse(ledger.serialize());
  if (data.entries && data.entries[1]) {
    data.entries[1].hash = "0".repeat(64);
  }
  const ledger2 = new BufferedLedger(clock, { hashChainEnabled: true });
  ledger2.deserialize(JSON.stringify(data));
  const broken = ledger2.verifyChain();
  assert(broken === 1, `Expected broken index 1, got ${broken}`);
});
