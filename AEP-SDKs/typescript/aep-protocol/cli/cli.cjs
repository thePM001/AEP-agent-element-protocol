...
function doctor() {
    ...
    // AEP-Policy-System reference GAP lattice
    const policiesDir = path.join(__dirname, '..', '..', '..', '..', 'AEP-Policy-System', 'reference');
    if (fs.existsSync(policiesDir)) {
        const polFiles = fs.readdirSync(policiesDir).filter(f => f.endsWith('.gap'));
        console.log(`  PASS: AEP-Policy-System/reference/  (${polFiles.length} .gap policies)`);
        passed++;
    } else {
        console.log(`  FAIL: AEP-Policy-System/reference/  - directory not found`);
        failed++;
    }
