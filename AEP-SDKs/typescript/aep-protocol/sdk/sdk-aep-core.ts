...
import * as jsyaml from "js-yaml"; // source-built dependency (no npm registry)
import { createMemoryEntry, type MemoryFabric } from "./sdk-aep-memory";
import {
  createDefaultMemoryFabric,
  isBaseNodeMemoryAvailable,
} from "./sdk-aep-memory-base-node";
...
export function loadAEPConfigs(configDir: string): AEPConfig {
  ...
  // LOW: pin safe schema (no custom JS types)
  const yamlOpts = { schema: jsyaml.CORE_SCHEMA };
  ...
  const latticeScene = validateLatticeScene(config);
  if (!latticeScene.valid) {
    throw new Error(
      `Lattice scene validation failed (mandatory for all systems): ${latticeScene.errors.join("; ")}`,
    );
  }
  return config;
}
...
export async function loadAEPConfigsBrowser(paths: {
  ...
  const yamlOpts = { schema: jsyaml.CORE_SCHEMA };
  const registryRaw = jsyaml.load(registryText, yamlOpts) as Record<string, any>;
  const theme = jsyaml.load(themeText, yamlOpts) as AEPTheme;
  const parsed = parseRegistryYAML(registryRaw);
...
export interface HyperlatticeBootValidation {
  valid: boolean;
  errors: string[];
}

/** Mandatory lattice topological matrix validation for every system type. */
export function validateLatticeScene(
  config: AEPConfig,
  memory?: MemoryFabric,
  hyperlatticeBoot?: HyperlatticeBootValidation,
): AEPValidationResult {
  const base = validateAOT(config, memory);
  if (!hyperlatticeBoot) return base;
  if (hyperlatticeBoot.valid) return base;
  return {
    valid: false,
    errors: [...base.errors, ...hyperlatticeBoot.errors],
  };
}
...
let cachedDefaultMemory: MemoryFabric | null | undefined;

function tryDefaultMemory(): MemoryFabric | undefined {
  if (cachedDefaultMemory !== undefined) {
    return cachedDefaultMemory ?? undefined;
  }
  if (isBaseNodeMemoryAvailable()) {
    cachedDefaultMemory = createDefaultMemoryFabric();
    return cachedDefaultMemory;
  }
  cachedDefaultMemory = null;
  return undefined;
}
...
export function validateAOT(
  ...
  // v2.0: record in memory if provided (auto Base Node fabric when configured)
  const fabric = memory ?? config.memory ?? tryDefaultMemory();
  if (fabric) {
...
export function validateJIT(
  ...
  // v2.0: record in memory if provided (auto Base Node fabric when configured)
  const jitFabric = memory ?? config.memory ?? tryDefaultMemory();
  if (jitFabric) {
    const entry = createMemoryEntry(
      elementId,
      "ui",
...
export {
  ...
  deriveEmbedding,
  isBaseNodeMemoryAvailable,
  resolveLatticeDbPath,
  resolveMemoryCliPath,
} from "./sdk-aep-memory-base-node";
export type {
  BaseNodeLatticeOptions,
  DynAepLatticeEvent,
  DynAepEventRecord,
  DockingPortWire,
} from "./sdk-aep-base-node-bridge";
export {
  BaseNodeLatticeLogger,
  createDefaultLatticeLogger,
  isBaseNodeLatticeAvailable,
  resolveLatticeLogCliPath,
} from "./sdk-aep-base-node-bridge";
export type { DockFrameResponse, DockingClientOptions } from "./sdk-aep-docking-client";
export {
  BaseNodeDockingClient,
  createDockingClient,
  resolveSocketBase,
  validationDockPath,
} from "./sdk-aep-docking-client";
export type { DockingPortId, DockingPortSpecWire } from "./sdk-aep-docking-spec";
export { docking_port_specs } from "./sdk-aep-docking-spec";
