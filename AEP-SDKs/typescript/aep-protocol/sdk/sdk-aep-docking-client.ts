...
function buildLatticeFrame(event: DynAepLatticeEvent): {
  ...
  const input = JSON.stringify({
    ...
    trust_score: event.trust_score ?? 100,
...
function frameWirePayload(sealed: {
  ...
  // Fail closed: Base Node binds wire key to registered agent; never send unbound frames
  if (!sealed.signer_public_hex || String(sealed.signer_public_hex).trim() === "") {
    throw new Error("signer_public_hex required on dock wire (fail-closed)");
  }
  return JSON.stringify({
    frame: sealed.frame,
    signer_public_hex: sealed.signer_public_hex,
  });
}
...
export class BaseNodeDockingClient {
  ...
  async pingAsync(): Promise<boolean> {
    ...
    // Fail closed: same wire shape as sync paths (signer required)
    const line = await sendLineAsync(
      this.validationPath,
      frameWirePayload(sealed),
    );
    const resp = JSON.parse(line) as DockFrameResponse;
    return resp.ok === true;
  }
