...
export class AgentGateway {
  ...
  private commercePolicies: Map<string, Record<string, unknown>> = new Map();
  ...
  createSessionFromPolicy(
    ...
    this.rollbackManager.setLedger(ledger, session.id); // H-53 per-session ledger
    ...
    if (policy.covenant) {
      try {
        ...
        // M-47: fail closed - session must not start without constraints when covenant configured
        throw new Error(
          `Covenant parse failed: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
    ...
    if (policy.commerce?.enabled) {
      this.commercePolicies.set(session.id, policy.commerce as Record<string, unknown>);
    }
  ...
  evaluate(sessionId: string, action: AgentAction): Verdict {
    ...
    const proposeDenial = checkProposeToken(action);
    if (proposeDenial) {
      session.recordAction("deny");
      ledger?.append("action:evaluate", {
        actionId: "propose-token-deny",
        tool: action.tool,
        decision: "deny",
        reasons: [proposeDenial],
        input: action.input,
      });
      return {
        decision: "deny",
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      // M-46: map policy decision into step verdict accurately
      // M-46: policy deny/gate must not be logged as pass for steps 1-12
      const stepVerdict =
        verdict.decision === "deny" ? "fail" :
        verdict.decision === "gate" ? "gate_pending" :
        "pass";
  ...
  /**
   * Validate a commerce action through the Rust commerce subprotocol (AEP-Subprotocols/commerce).
   */
  validateCommerce(
    sessionId: string,
    action: string,
    ...
    const policy = this.commercePolicies.get(sessionId);
    // HIGH: fail closed without bound commerce policy (was valid:true)
    if (!policy) {
      return {
        valid: false,
        errors: [
          `No commerce policy bound for session ${sessionId}; validateCommerce denied (fail-closed).`,
        ],
      };
    }
    return validateCommerceRust(action, payload, policy);
  }
  ...
  rollback(sessionId: string, actionId: string): RollbackResult {
    // H-52: only roll back actions that belong to this session
    const sessionPlans = this.rollbackManager.getSessionPlans(sessionId);
    if (!sessionPlans.some((p) => p.actionId === actionId)) {
      return {
        actionId,
        success: false,
        compensationApplied: null,
        error: `Action "${actionId}" is not registered on session "${sessionId}".`,
      };
    }
  ...
  terminateSession(sessionId: string, reason: string): SessionReport {
    ...
    this.commercePolicies.delete(sessionId);
