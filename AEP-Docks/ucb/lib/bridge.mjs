...
export async function ingestForeignPayload(body, runtime, context = {}) {
  // TM-18: parity with Rust aep-ucb - task_manifest required (fail closed).
  if (!body || typeof body !== "object" || !body.task_manifest) {
    return {
      ok: false,
      status: "rejected",
      error: "task_manifest required: provide task_manifest in ingest body (deprecated MJS UCB; prefer aep-ucb)",
      validation: { ok: false, reason: "task_manifest_required" },
    };
  }
