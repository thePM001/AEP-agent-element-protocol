...
async fn health_handler(State(state): State<AppState>) -> Json<Value> {
    // LOW: public health is coarse only (no dock/lattice diagnostics)
    let full = health_snapshot(&state.runtime).await;
    Json(json!({
        "service": full.get("service").cloned().unwrap_or(json!("ucb-universal-connect-bridge")),
        "version": full.get("version").cloned().unwrap_or(json!(UCB_VERSION)),
        "status": full.get("status").cloned().unwrap_or(json!("unknown")),
    }))
}
...
async fn rollback_handler(
    ...
    let steps = match &body.steps {
        Some(Value::Number(n)) if n.is_u64() => n.as_u64().unwrap_or(1),
        Some(Value::Number(n)) if n.is_i64() => n.as_i64().unwrap_or(1).max(0) as u64,
        _ => {
            return (
    ...
    let Ok(steps) = crate::mcp::clamp_rollback_steps(steps) else {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({
                "ok": false,
    ...
    let result = rollback_foreign_integrations(&state.runtime, steps).await;
    let status = if result.get("ok") == Some(&json!(true)) {
        StatusCode::OK
    } else {
...
async fn egress_proxy_handler(
    ...
    let agent_id = match headers
        ...
        .filter(|s| !s.is_empty())
    {
        Some(id) if id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_' || c == '.') => {
            id.to_string()
        }
        Some(_) => {
            return (
                ...
                Json(json!({
                    "ok": false,
                    "error": "invalid X-AEP-Agent-Id (allowed: A-Za-z0-9._-)"
                })),
            )
                .into_response();
        }
        None => {
            return (
                ...
                Json(json!({
                    "ok": false,
                    "error": "X-AEP-Agent-Id header required for egress (no default agent spoof)"
                })),
            )
                .into_response();
        }
    ...
    // CRITICAL: provisional manifests must not drive live egress (SSRF plant path).
    if let Some(ref m) = manifest {
        if m.provisional {
            return (
                ...
                Json(json!({
                    "ok": false,
                    "error": "provisional task manifest cannot use egress until promoted"
                })),
            )
                .into_response();
        }
