...
pub async fn ingest_foreign_payload(
    ...
    .await
    {
        ...
        Err(SynthesisError::AgentIdMismatch) => {
            return json!({
                "ok": false,
                "status": "rejected",
                "error": SynthesisError::AgentIdMismatch.to_string(),
            });
        }
    };

    // TM-22: lattice frame trust cannot exceed manifest max or foreign hard max.
    event.trust_score = event
        .trust_score
        .min(manifest.trust.max_trust_score)
        .min(crate::ingress::FOREIGN_TRUST_HARD_MAX);
...
pub async fn rollback_foreign_integrations(rt: &Arc<UcbRuntime>, steps: usize) -> Value {
    ...
    let rollback_event = crate::translator::LatticeEvent {
        ...
        // TM-22: rollback is bridge-internal; keep provisional tier (no elevated stamp).
