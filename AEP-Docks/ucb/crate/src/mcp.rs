    ...
    match method {
        ...
        "tools/call" => {
            ...
            let result = match name {
                ...
                "ucb_rollback" => {
                    // BM-04: clamp steps to 1..=100 (same as HTTP rollback_handler)
                    let raw = args.get("steps").and_then(|v| v.as_u64()).unwrap_or(1);
                    let steps = match clamp_rollback_steps(raw) {
                        Ok(s) => s,
                        Err(_) => {
                            return json!({
                                "jsonrpc": "2.0",
...
#[cfg(test)]
mod tests {
    use super::clamp_rollback_steps;
