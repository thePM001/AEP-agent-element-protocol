...
- Base Node **always** enforces task manifests on docks (strict identity is not optional; `AEP_DOCK_STRICT_IDENTITY` is legacy/ignored)
...
Manifests written to `AEP_TASK_MANIFEST_DIR` (default `$AEP_DATA/ucb/manifests`). Base Node and UCB share `AEP_TASK_MANIFEST_DIR` (default `$AEP_DATA/ucb/manifests`).
...
| `AEP_DOCK_STRICT_IDENTITY` | ignored | Legacy; Base Node dock identity is always strict |
