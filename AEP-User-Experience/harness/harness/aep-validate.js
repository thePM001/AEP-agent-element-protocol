...
// The one writing rule table, generated by the compiled wall set in the admit
// crate. Every prose dash verdict in this harness reads this table.
const WRITING_RULE_TABLE = (() => {
    const tablePath = path.join(__dirname, '..', '..', '..', 'AEP-Components', 'admit', 'rules', 'writing-rules.json');
    return JSON.parse(fs.readFileSync(tablePath, 'utf8')).rules;
})();
...
class AEPValidator {
    ...
    // -----------------------------------------------------------------------
    // Check 8: Forbidden typographic dashes
    // -----------------------------------------------------------------------
    checkDashes(file, content) {
        // The one writing rule table decides the forbidden characters. This
        // harness reads the table instead of holding a private character list.
        for (const rule of WRITING_RULE_TABLE) {
            if (rule.id !== "no_em_dashes" && rule.id !== "no_en_dashes") continue;
            const code = rule.id === "no_em_dashes" ? "EM_DASH" : "EN_DASH";
            for (const point of rule.code_points ?? []) {
                const ch = String.fromCodePoint(point);
                const label = `U+${point.toString(16).toUpperCase()}`;
                let idx = content.indexOf(ch);
                while (idx !== -1) {
                    const lineNum = content.substring(0, idx).split("\n").length;
                    this.addViolation(SEVERITY.LOW, file, lineNum, code,
                        `Forbidden dash ${label} found. Use - instead.`);
