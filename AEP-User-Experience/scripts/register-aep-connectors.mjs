#!/usr/bin/env node
/** Register internal-export-area/AEP-Connectors/catalog.json entries in AEP-Base-Node registry */
...
const catalog = JSON.parse(readFileSync(join(REPO, "internal-export-area/AEP-Connectors/catalog.json"), "utf8"));
...
function manifestFor(spec) {
  ...
  return {
    ...
    version: "2.8.0",
    kind: "connector",
    path: `internal-export-area/AEP-Connectors/${folder}/`,
    ...
    implementation: {
      module: `internal-export-area/AEP-Connectors/${folder}/lib/${spec.service}-connector.mjs`,
...
for (const spec of catalog.connectors) {
  ...
  if (!existingIds.has(spec.id)) {
    registryCatalog.components.push({
      ...
      path: `internal-export-area/AEP-Connectors/${spec.folder}/`,
    ...
    if (entry) {
      entry.path = `internal-export-area/AEP-Connectors/${spec.folder}/`;
