...
name: aep
description: >-
  Use this skill whenever working with AEP (Agent Element Protocol) 2.8, dynAEP
  (main AEP event runtime), Base Node, Composer Lite, CCA / setup agent, component
  registry, CAW, UCB, Path A/B connect, dynAEP-TA, dynAEP-TA-P or any AEP
  governance feature. Triggers include 'AEP', 'dynAEP', 'dynAEP-TA', 'dynAEP-TA-P',
  'temporal authority', 'perception governance', 'perception registry', 'bridge
  clock', 'causal ordering', 'vector clock', 'TimesFM', 'adaptive perception',
  'perception bounds', 'scene graph', 'aep-scene.json', 'aep-registry.yaml',
  'aep-theme.yaml', 'zero-trust UI', 'topological matrix', 'z-band', 'skin
  binding', 'AEP-FCR', 'temporal annotations', 'speech pacing', 'haptic timing',
  'notification cadence', 'Schema Builder', 'Policy Builder', 'Lattice Memory',
  'evaluation chain', 'trust scoring', 'execution rings', 'behavioural covenants',
  'content scanners', 'evidence ledger', 'CCA', 'aep-cca', 'aep-setup-agent',
  'Composer Lite', 'CAW', 'aep-caw', 'UCB', 'secure deployment' or building
  validated UI for AI agents. Also use when implementing AEP three-layer
  architecture, writing AEP validators, creating MCP servers that validate agent
  UI output, working with AG-UI under AEP governance or governing time-dependent
  outputs for human perception. If AEP MCP tools are available (list_aep_schemas,
  create_ui_element, get_scene_graph), always consult this skill first. Do NOT
  guess IDs, skin bindings, z-bands or element types. Do NOT use Date.now() or any
  local clock when dynAEP-TA is available - call dynaep_temporal_query instead.
---
...
**Operators:** follow the **[AEP secure deployment guide](../docs/AEP-2.8-SECURE-DEPLOYMENT-GUIDE.md)** for Path A (native dynAEP + lattice) and Path B (optional UCB foreign attach). Do not invent a second protocol runtime to replace dynAEP. Do not hand foreign stacks raw Base Node dock sockets.
...
cp .env.example .env
# Required: COMPOSER_LITE_SETUP_TOKEN (openssl rand -hex 32). Prefer UCB_API_KEY when UCB=1.
docker compose -f docker-compose.public.yml up -d --build
# Install wizard / Composer Lite
open http://localhost:8424/install

# CCA (Central Setup Agent) - preferred operator path after Base Node is up
docker compose -f docker-compose.public.yml exec aep aep-cca probe
docker compose -f docker-compose.public.yml exec aep aep-cca plan --intent "your deployment intent"
docker compose -f docker-compose.public.yml exec aep aep-cca execute
# Equivalent wrapper (same CCA setup-agent entry):
# docker compose -f docker-compose.public.yml exec aep aep-setup-agent

# Host-side checks (from repo root)
node AEP-User-Experience/aep-base-node-preflight.mjs
node AEP-User-Experience/aep-validate.js
# Nested harness copy (UI scene/registry/theme):
node AEP-User-Experience/harness/harness/aep-validate.js
./AEP-Components/conformance/runner/run.sh
```

## Connect model (Path A / Path B)

| Path | When | How |
|------|------|-----|
| **Path A (preferred)** | AEP-aware agents and native clients | dynAEP + AEP SDKs / `lattice-transport` sealed frames to Base Node docks. Register identity. Prefer lattice-gated outbound HTTP. Enable CAW for shell/file. |
| **Path B (optional)** | Foreign non-AEP frameworks | UCB on loopback/private (`:8412`), strong `UCB_API_KEY`, **task manifest required**. UCB uses lattice internally. Never mount raw dock sockets into the foreign stack. Set `UCB=0` if unused. |

Canonical connect detail: [AEP secure deployment guide](../docs/AEP-2.8-SECURE-DEPLOYMENT-GUIDE.md).

## CCA (Central Setup Agent)

CCA lives at `AEP-Components/cca/`. After Docker deploy and Base Node startup it:

1. Probes the environment (CPU, RAM, disk, Docker, constraints)
2. Loads the component registry and manifests
3. Accepts natural-language deployment intent
4. Produces an **ImplementationPlan** and executes activation (LRPs, connectors, policies, lattice registration)
5. Syncs the plan graph with Composer Lite

CLI: `aep-cca` (Docker: `docker compose exec aep aep-cca ...`). Wrapper: `aep-setup-agent` runs `AEP-Components/cca/setup-agent.mjs`.

Default public-tier model: **no dedicated external validation engine** is required. Base Node + dynAEP lattice governance + EPSCOM/writing enforcement run in-tree. Optional validation-engine modes are operator opt-in only (see CCA docs). Do not assume a separate third-party validation dock is installed.

## OpenAI Codex installation

Codex reads `AGENTS.md` and MCP config in `.codex/config.toml`.

```bash
# After Base Node activation:
npx aep init codex
codex mcp list
codex # then /mcp to verify aep server
```

Creates `AGENTS.md`, agent policy and `.codex/config.toml` with `npx aep proxy --policy ./agent.policy.yaml`. Full guide: `AEP-User-Experience/docs/CODEX-INSTALL.md`.

## Composer Lite connector node

Connector (NT-00006, diamond) bridges external applications into the Action Lattice. Palette quick slot `builtin-connector`, right-click **Add Connector**, inspector fields `hub_connector_id` and `storage_backend`.
...
**Subsystem**: src/economics/ (9 TypeScript modules) + internal-sdk/AEP-Subprotocols/commerce/model-mapping.yaml and price-catalog.yaml
...
| File | Purpose |
|------|---------|
| internal-sdk/AEP-Subprotocols/commerce/model-mapping.yaml | 10 canonical models mapped to provider-specific IDs |
| internal-sdk/AEP-Subprotocols/commerce/price-catalog.yaml | Per-million-token pricing for 10+ providers with capabilities |
...
Seven subprotocols in **`internal-sdk/AEP-Subprotocols/`** (Rust crates, catalog pattern like `AEP-Base-Node/registry/`). CLI: `aep-subprotocol`. Docs: `internal-sdk/AEP-Subprotocols/README.md`, `AEP-User-Experience/docs/SUBPROTOCOLS.md`.

| Subprotocol | Crate | What It Validates |
|-------------|-------|-------------------|
| UI | `aep-subprotocol-ui` | Scene graph, FCR behaviour, theme skin bindings |
| Workflows | `aep-subprotocol-workflows` | Actions, state transitions, payload schemas |
| REST APIs | `aep-subprotocol-rest-api` | HTTP methods, paths, bodies, headers |
| Events | `aep-subprotocol-events` | Topics, payloads, producers, correlation IDs |
| IaC | `aep-subprotocol-iac` | Resource kinds, required/forbidden fields |
...
UI three-layer files: `internal-sdk/AEP-Subprotocols/ui/aep-scene.json`, `aep-registry.yaml`, `aep-theme.yaml`.

---

## dynAEP: Dynamic Agent Element Protocol (main AEP event runtime)

**dynAEP is the main AEP event runtime in 2.8.** It ships in-tree at `AEP-Components/dynAEP/` with clients under `internal-sdk/AEP-SDKs/typescript/dynaep/`. Do not invent a second protocol runtime to replace it. Foreign frameworks attach via Path A (native lattice) or Path B (UCB); they do not substitute for dynAEP.
...
**dynAEP** solves the runtime problem: every live AG-UI event is validated against the AEP graph and Action Lattice before downstream effects.
...
## Local AEP 2.8 stack (offline)

```bash
cp .env.example .env
# set COMPOSER_LITE_SETUP_TOKEN; set UCB_API_KEY if UCB=1
docker compose -f docker-compose.public.yml up -d --build
open http://localhost:8424/install
docker compose -f docker-compose.public.yml exec aep aep-cca probe
# or: docker compose -f docker-compose.public.yml exec aep aep-setup-agent
```

- Component registry: `AEP-Base-Node/registry/catalog.json`
- dynAEP runtime: `AEP-Components/dynAEP/` and `internal-sdk/AEP-SDKs/typescript/dynaep/`
...
- AEP (public): https://github.com/thePM001/AEP-agent-element-protocol
- AEP secure deployment guide: `AEP-User-Experience/docs/AEP-2.8-SECURE-DEPLOYMENT-GUIDE.md`
- dynAEP in-tree: `AEP-Components/dynAEP/` (merged hyperlattice runtime)
- dynAEP historical repo: https://github.com/thePM001/dynAEP-dynamic-agent-element-protocol
