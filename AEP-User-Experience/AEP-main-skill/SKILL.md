...
AEP 2.8 adds a mandatory local **Base Node** daemon, merged **dynAEP**, **Composer Lite** (port 8424), **setup agent**, **component registry** and compliance LRP modules. Docker ships the full offline protocol. Public repo: https://github.com/thePM001/AEP-agent-element-protocol (publish when ready).

## AEP 2.8 stack

| Component | Path | Role |
|-----------|------|------|
| Base Node | `AEP-Base-Node/crate` | Local governance daemon, four docking ports |
| dynAEP | `dynAEP/` | Action Lattice event governance |
| Setup agent | `setup-agent/` | Activation, inference, LRP and registry |
| Composer Lite | `composer-lite/` | WASM visual canvas `:8424` |
| Registry | `registry/` | Offline component catalog |
| CAW Framework | `caw-framework/` | Execution-Layer Security (`aep-caw`, core capability) |
| Harness | `AEP-User-Experience/harness/` | Agent preflight and validation |
...
2. **Validation engine plan** - acquire NLA-built engine, build your own or try without (advantages listed for each)

Non-interactive: `AEP_INSTALL_METHOD`, `AEP_VALIDATION_ENGINE`, `AEP_DATA`. See `AEP-User-Experience/docs/SETUP-AGENT.md`.
...
Creates `AGENTS.md`, `agent.policy.yaml` and `.codex/config.toml` with `npx aep proxy --policy ./agent.policy.yaml`. Full guide: `AEP-User-Experience/docs/CODEX-INSTALL.md`.
...
### Harness
/harness/aep-economics.js wires all economics modules for agent integration.
...
Seven subprotocols in **`AEP-Subprotocols/`** (Rust crates, catalog pattern like `registry/`). CLI: `aep-subprotocol`. Docs: `AEP-Subprotocols/README.md`, `AEP-User-Experience/docs/SUBPROTOCOLS.md`.
...
Component registry: `AEP-Base-Node/registry/catalog.json`. Optional GitHub catalog merge: `AEP_COMPONENTS_FETCH=1`.
