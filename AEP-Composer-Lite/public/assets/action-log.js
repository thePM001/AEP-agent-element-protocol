...
export class ComposerActionLog {
  ...
  _appendObjectSection(category, groupNodes, selectedId) {
    ...
    for (const node of groupNodes) {
      ...
      typeEl.className = "action-log-object-type";
      typeEl.style.setProperty("--object-type-color", typeColor);
      typeEl.textContent = typeLabel(node.type);
      const idEl = document.createElement("span");
      idEl.className = "action-log-object-id";
  ...
  render(timeline = []) {
    ...
    for (let i = entries.length - 1; i >= 0; i -= 1) {
      ...
      icon.className = "action-log-item-icon";
      icon.setAttribute("aria-hidden", "true");
      icon.textContent = KIND_ICONS[entry.kind] || "•";
      const label = document.createElement("span");
      label.className = "action-log-item-label";
