...
function renderAgentstreamRingNode(el, node, layout) {
  ...
  const safeId = String(node.id ?? "node").replace(/[^a-zA-Z0-9_-]/g, "_");
  el.innerHTML = `
    ${portButtonHtml("in")}
    ${portButtonHtml("out")}
    <svg class="agentstream-ring-svg" viewBox="0 0 ${w} ${h}" aria-hidden="true">
      <defs>
        <radialGradient id="as-ring-fill-${safeId}" cx="50%" cy="50%" r="65%">
        ...
        <filter id="as-ring-glow-${safeId}" x="-40%" y="-40%" width="180%" height="180%">
      ...
      <path class="agentstream-ring-body" fill="url(#as-ring-fill-${safeId})" filter="url(#as-ring-glow-${safeId})"
        d="M ${cx + outerRx} ${cy}
