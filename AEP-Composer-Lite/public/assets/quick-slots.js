...
export function initQuickSlots(grid, palette, options = {}) {
  ...
  function render() {
    ...
    for (let i = 0; i < SLOT_COUNT; i += 1) {
      ...
      if (item) {
        btn.draggable = true;
        const color = String(item.color || "#888").replace(/[^-#a-zA-Z0-9(),.% ]/g, "");
        const glyph = shortLabel(item).replace(/[<>&"']/g, "");
        const label = String(item.label ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
        btn.innerHTML = `
          <span class="quickslot-disc">
            <span class="quickslot-core" style="--slot-color:${color}"></span>
            <span class="quickslot-glyph"></span>
            <span class="quickslot-key">${i + 1}</span>
          </span>
          <span class="quickslot-label"></span>
        `;
        btn.querySelector(".quickslot-glyph").textContent = glyph;
        btn.querySelector(".quickslot-label").textContent = item.label ?? "";
