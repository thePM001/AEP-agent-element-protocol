...
function renderPanel() {
  ...
  if (state.step === 0) {
    const health = escapeHtml(state.status.health?.status ?? "unknown");
    ...
    panel.innerHTML = `
      ...
      <p class="lead">Install method for this container: <strong>Docker Compose (recommended)</strong>. Data dir: <code>${dataDir}</code></p>
      <div class="field">
        ...
        <select id="validation-engine">
          ${state.catalog.validation_engines
            .map(
              (v) =>
                `<option value="${escapeHtml(v.id)}" ${state.validation_engine === v.id ? "selected" : ""}>${escapeHtml(v.label)}</option>`,
            )
            .join("")}
        </select>
      </div>`;
