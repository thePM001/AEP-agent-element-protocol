...
class LiteInventory {
  ...
  _safeColor(c) {
    return String(c || "#888").replace(/[^-#a-zA-Z0-9(),.% ]/g, "");
  }

  _pickerItemHtml(entry) {
    const id = this._esc(entry.id);
    const name = this._esc(entry.name);
    const color = this._safeColor(entry.color);
    const glyph = this._esc(entry.short || String(entry.name || "").slice(0, 2));
    const meta = this._esc(`${String(entry.baseType || "").toUpperCase()} · ${this.catalogShape(entry)}`);
    return `<button type="button" class="quickslot-picker-item" data-catalog-id="${id}" draggable="true" data-lite-tip="${name}" aria-label="${name}" style="--slot-color:${color}">
      <span class="quickslot-picker-core" style="background:${color}"></span>
      <span class="quickslot-picker-glyph">${glyph}</span>
      <span class="quickslot-picker-name">${name}</span>
      <span class="quickslot-picker-meta">${meta}</span>
    </button>`;
  }
  ...
  _catalogCard(entry) {
    ...
    const shape = this._esc(this.catalogShape(entry));
    const registry = this._esc(entry.registry_id || entry.catalog_id || entry.id);
    const color = this._safeColor(entry.color);
    const baseType = this._esc(entry.baseType);
    const id = this._esc(entry.id);
    const name = this._esc(entry.name);
    const glyph = this._esc(entry.short || String(entry.name || "").slice(0, 2));
    const shapeControl = `<div class="inventory-shape-field">
      <span class="inventory-shape-label">Shape</span>
      <div class="shape-picker shape-picker-catalog" data-shape-picker data-base-type="${baseType}" data-shape-value="${shape}" data-shape-color="${color}"></div>
    </div>`;
    return `<article class="inventory-card" data-catalog-id="${id}" draggable="true">
      <div class="inventory-card-icon" style="--slot-color:${color}">
        <span class="inventory-card-core" style="background:${color}"></span>
        <span class="inventory-card-glyph">${glyph}</span>
      </div>
      <div class="inventory-card-body">
        <div class="inventory-card-name">${name}</div>
        <div class="inventory-card-meta">${baseType.toUpperCase()} · ${registry}</div>
        ${shapeControl}
