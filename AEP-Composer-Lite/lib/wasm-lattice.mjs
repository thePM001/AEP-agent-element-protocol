...
export async function wasmLatticeEvaluate(runtime, body) {
  ...
  const socketPath = wasmSandboxSocket(runtime.socketBase);
  // HIGH residual: must bind dock signer; bare frame is rejected by Base Node binding
  if (!recorded.signer_public_hex) {
    throw new Error("WASM evaluate missing sealed signer_public_hex (fail-closed)");
  }
  return sendLatticeFrame(socketPath, recorded.frame, {
    trustScore: 700,
    signerPublicHex: recorded.signer_public_hex,
  });
}
